...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void TestCustomInputOutputOperations(void)
{
	PrintLineSeparator();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void MyUserDefinedCallback(ios_base::event Type, ios_base& Stream, int Index)
{
	wstring														EventType=L"UNKNOWN EVENT";

	if (Type==ios_base::erase_event)
	{
		EventType=L"ERASE EVENT";
	}
	else if (Type==ios_base::imbue_event)
	{
		EventType=L"IMBUE EVENT";
	}
	else if (Type==ios_base::copyfmt_event)
	{
		EventType=L"COPYFMT EVENT";
	}

	wcout << L"!!!!! USER DEFINED CALLBACK with EVENT [" << EventType << L"] CALLED !" << endl;

	// !!! Pri volanie ERASE EVENT je NUTNE odstranit CUSTOM DATA volanim OPERATOR DELETE, pretoze v CUSTOM MANIPULATOR boli CUSTOM DATA alokovane volanim OPERATOR NEW.
	if (Type==ios_base::erase_event)
	{
		std::wstring*											ValueVoidPointer=static_cast<std::wstring*>(Stream.pword(Index));

		if (ValueVoidPointer!=nullptr)
		{
			delete(ValueVoidPointer);

			Stream.pword(Index)=nullptr;
		}
	}
	// !!! Pri volani METHOD basic_ios::copyfmt() je NUTNE vykonat DEEP COPY CUSTOM DATA.
	else if (Type==ios_base::copyfmt_event)
	{
		// !!!!! Pri volani EVENT COPYFMT uz DOSLO k vytvorenie SHALLOW COPY CUSTOM DATA ARRAY a ulohou USER DEFINED CALLBACK je vytvorit DEEP COPY zo SHALLOW COPY.
		std::wstring*											ValueVoidPointer=static_cast<std::wstring*>(Stream.pword(Index));

		if (ValueVoidPointer!=nullptr)
		{
			// !!!!! Vytvori sa DEEP COPY povodnych CUSTOM DATA.
			std::wstring*										NewValueVoidPointer=new wstring(*ValueVoidPointer);

			// !!!!! Povodne CUSTOM DATA sa nahradia KOPIOU.
			Stream.pword(Index)=NewValueVoidPointer;
		}
	}
}
//-----------------------------------------------------------------------------
void TestCustomInputOutputOperationsWithCustomData(void)
{
	PrintLineSeparator();

	{
		CCustomInputOutputOperatorsDerived						Object(111,222.345,333,444.567);

		wcout << L"OBJECT [" << Object.ToString() << L"] !" << endl;

		wstringstream											StringStream;

		// !!! Nastavia sa CUSTOM DATA TYPE [int].
		AddIntegerCustomData(StringStream,100);
...

	{
		...
		// !!!!! Kedze do STREAM sa prida CUSTOM DATA TYPE [void*], ktore su alokovane v MANIPULATOR, je NEVYHNUTNE do C++ STREAM pridat USER DEFINED CALLBACK, aby alokovany BUFFER v CUSTOM DATA TYPE [void*] uvolnil.
		// !!! Ako 2. PARAMETER sa prida INDEX na CUSTOM DATA TYPE [void*].
		StringStream.register_callback(MyUserDefinedCallback,CUSTOM_DATA_VOID_POINTER_INDEX);

		// !!! Nastavia sa CUSTOM DATA TYPE [void*].
		AddVoidPointerCustomData(StringStream,L"ABCDE");
...

	{
		...
		// !!!!! Kedze do STREAM sa prida CUSTOM DATA TYPE [void*], ktore su alokovane v MANIPULATOR, je NEVYHNUTNE do C++ STREAM pridat USER DEFINED CALLBACK, aby alokovany BUFFER v CUSTOM DATA TYPE [void*] uvolnil.
		// !!! Ako 2. PARAMETER sa prida INDEX na CUSTOM DATA TYPE [void*].
		StringStream1.register_callback(MyUserDefinedCallback,CUSTOM_DATA_VOID_POINTER_INDEX);

		// !!! Nastavia sa CUSTOM DATA TYPE [int].
		AddIntegerCustomData(StringStream1,100);

		// !!! Nastavia sa CUSTOM DATA TYPE [void*].
		AddVoidPointerCustomData(StringStream1,L"ABCDE");

		// !!! Nastavi sa CUSTOM WIDTH a FILL CHARACTER.
		StringStream1 << setw(50);
		StringStream1 << setfill(L'_');

		wstringstream											StringStream2;

		// !!!!! Pouzije sa METHOD basic_ios::copyfmt() pri ktorej sa robi DEEP COPY CUSTOM DATA.
		StringStream2.copyfmt(StringStream1);

		// !!! Vykona sa zapis CUSTOM OBJECT do STREAM 1.
		StringStream1 << Object;

		wcout << L"STREAM CONTENT [" << StringStream1.str() << L"] !" << endl;

		// !!! Vykona sa zapis CUSTOM OBJECT do STREAM 1.
		StringStream2 << Object;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void TestCustomOutputStreamBufferNoBuffering(void)
{
	PrintLineSeparator();

	{
		CCustomOutputStreamBufferNoBuffering1					CustomBuffer;

		// !!! STREAM pouzije CUSTOM OUTPUT STREAM BUFFER.
		wostream												Stream(&CustomBuffer);
		...
		wcout << L"CUSTOM STREAM BUFFER CONTENT [" << CustomBuffer.GetDataStore() << L"] !" << endl;
	}

	PrintLineSeparator();

	{
		// !!! Pouzije sa CUSTOM STREAM, ktory pouziva CUSTOM STREAM BUFFER.
		CCustomOutputStreamNoBuffering1							CustomStream;

		CustomStream << L"ABC";
		CustomStream << L" ";
		CustomStream << 123;
		CustomStream << L" ";
		CustomStream << 456.789;
		CustomStream << L" ";
		CustomStream << L"XYZ !!!";

		wcout << L"CUSTOM STREAM BUFFER CONTENT [" << CustomStream.GetStreamContent() << L"] !" << endl;
	}

	PrintLineSeparator();

	{
		CCustomOutputStreamBufferNoBuffering2					CustomBuffer;
	...
	{
		// !!! Pouzije sa CUSTOM STREAM, ktory pouziva CUSTOM STREAM BUFFER.
		CCustomOutputStreamNoBuffering2							CustomStream;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void TestCustomOutputStreamBufferWithBuffering(void)
{
	PrintLineSeparator();

	{
		CCustomOutputStreamBufferWithBuffering					CustomBuffer;
		...
		// !!! Spravi sa EXPLICITNY FLUSH, aby sa vsetky CHARACTERS z CUSTOM BUFFER preniesli do DATA STORE.
		Stream << flush;
	...
	{
		// !!! Pouzije sa CUSTOM STREAM, ktory pouziva CUSTOM STREAM BUFFER.
		CCustomOutputStreamWithBuffering						CustomStream;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void TestCustomInputStreamBuffer(void)
{
	PrintLineSeparator();

	{
		CCustomInputStreamBuffer								CustomBuffer(L"ABC 123 456.789 XYZ !!!");

		// !!! STREAM pouzije CUSTOM INPUT STREAM BUFFER.
		wistream												Stream(&CustomBuffer);
		wstring													Value1;
		...
		// !!! Do STREAM sa vratia 2 CHARACTERS.
		Stream.unget();
		Stream.unget();
		...
		if (Stream.operator bool()==true)
		{
			wcout << L"VALUE 5 [" << Value5 << L"] READ from STREAM !" << endl;
		}
		else
		{
			wcout << L"ERROR OCCURED or END OF FILE REACHED !" << endl;
		...
		if (Stream.operator bool()==true)
		{
			wcout << L"VALUE 6 [" << Value6 << L"] READ from STREAM !" << endl;
		}
		else
		{
			wcout << L"ERROR OCCURED or END OF FILE REACHED !" << endl;
	...
	{
		CCustomInputStreamBuffer								CustomBuffer(L"ABCD 12345");
		...
		// !!! Do STREAM sa vrati CHARACTER.
		Stream.unget();

		// !!! Do STREAM sa vrati SPRAVNY CHARACTER.
		Stream.putback(L'C');

		// !!!!! Do STREAM sa vrati NESPRAVNY CHARACTER.
		// !!! STREAM BUFFER zavola METHOD pbackfail().
		Stream.putback(L'!');
...

	{
		CCustomInputStreamBuffer								CustomBuffer(L"ABCD 12345");
		wchar_t													Buffer1[6];
		streamsize												NumberOfBytesRead1=CustomBuffer.sgetn(Buffer1,sizeof(Buffer1)/sizeof(Buffer1[0]));

		if (NumberOfBytesRead1>0)
		{
			wstring												String(Buffer1,NumberOfBytesRead1);

			wcout << L"VALUE 1 [" << String << L"] READ from STREAM !" << endl;
		}
		else
		{
			wcout << L"ERROR OCCURED or END OF FILE REACHED !" << endl;
		...
		wchar_t													Buffer2[6];
		streamsize												NumberOfBytesRead2=CustomBuffer.sgetn(Buffer2,sizeof(Buffer2)/sizeof(Buffer2[0]));

		if (NumberOfBytesRead2>0)
		{
			wstring												String(Buffer2,NumberOfBytesRead2);

			wcout << L"VALUE 2 [" << String << L"] READ from STREAM !" << endl;
		}
		else
		{
			wcout << L"ERROR OCCURED or END OF FILE REACHED !" << endl;
...
int main(void)
{
	...
	//TestLocale();
	//TestCustomInputOutputOperations();
	//TestCustomInputOutputOperationsWithCustomData();
	//TestTiedStreams();
	//TestRedirectedStreams();
	//TestStreamBufferIterators();
	//TestCustomOutputStreamBufferNoBuffering();
	//TestCustomOutputStreamBufferWithBuffering();
	//TestCustomInputStreamBuffer();
	//TestStringStream();
