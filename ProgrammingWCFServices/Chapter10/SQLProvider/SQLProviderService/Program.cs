...
//-------------------------------------------------------------------------------------------------------
		static void Main(string[] args)
		{
			...
			CServiceHostWrapper<CSQLProviderService>		Service=new CServiceHostWrapper<CSQLProviderService>();
			...
			Service.Host.Credentials.ServiceCertificate.SetCertificate(StoreLocation.LocalMachine,StoreName.TrustedPeople,X509FindType.FindBySubjectName,"TestSQL");

			Service.AddServiceEndpoint<ISQLProviderContract>(TCPBinding,"net.tcp://localhost:9001/ISQLProviderContract");

			Service.AddTCPMEXEndpoint("net.tcp://localhost:9001/MEX");

			ServiceHostWrapperCollection.StartServices();

			// Musim to dat az sem, lebo sluzba musi byt nastartovana aby boli providers inicializovane.

			// !!!!! Na to aby program fungoval MUSIM VYTVORIT USERS a ROLES metodami CreateUser() a CreateRole().

			// Vytvorenie usera.
			//CreateUser();
			//DeleteUser();

			// Vytvorenie roly a pridanie usera do roly.
			//CreateRole();
			//DeleteRole();
