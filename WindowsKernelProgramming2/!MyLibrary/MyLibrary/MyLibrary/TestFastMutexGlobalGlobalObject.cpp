...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
ULONG64															FastMutexGlobalGlobalObjectSharedVariable=0;
CFastMutexGlobal												FastMutexGlobalGlobalObject;
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexGlobalGlobalObject1(PVOID StartContext)
{
	...
	for(LONG64 Index=0;Index<LONG64(1000*1000);Index++)
	{
		...
		{
			FastMutexGlobalGlobalObjectSharedVariable++;
		}
	}
...
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexGlobalGlobalObject2(PVOID StartContext)
{
	const char*													FunctionName=__FUNCTION__;
...
NTSTATUS TestFastMutexGlobalGlobalObject(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
	...
	{
		FastMutexGlobalGlobalObject.Init();

		{
			...
			do
			{
				Status=CHelpers::CreateThread(DeviceObject,"111",ThreadFastMutexGlobalGlobalObject1,&FastMutexGlobalGlobalObject,&ThreadObject1);
				...
				Status=CHelpers::CreateThread(DeviceObject,"222",ThreadFastMutexGlobalGlobalObject2,&FastMutexGlobalGlobalObject,&ThreadObject2);
				...
				KdPrint(("!!!!!!!!!! ROUTINE [%s] - SHARED VARIABLE [%I64d].\n",FunctionName,FastMutexGlobalGlobalObjectSharedVariable));
			}
			while(false);
