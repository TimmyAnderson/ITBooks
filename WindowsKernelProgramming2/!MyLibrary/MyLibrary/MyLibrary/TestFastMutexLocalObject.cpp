...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
ULONG64															FastMutexLocalObjectSharedVariable=0;
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexLocalObject1(PVOID StartContext)
{
	...
	for(LONG64 Index=0;Index<LONG64(1000*1000);Index++)
	{
		...
		{
			FastMutexLocalObjectSharedVariable++;
		}
	}
...
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexLocalObject2(PVOID StartContext)
{
	const char*													FunctionName=__FUNCTION__;
...
NTSTATUS TestFastMutexLocalObject(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
	...
	{
		CFastMutex												FastMutexLocalObject;

		{
			...
			do
			{
				Status=CHelpers::CreateThread(DeviceObject,"111",ThreadFastMutexLocalObject1,&FastMutexLocalObject,&ThreadObject1);
				...
				Status=CHelpers::CreateThread(DeviceObject,"222",ThreadFastMutexLocalObject2,&FastMutexLocalObject,&ThreadObject2);
				...
				KdPrint(("!!!!!!!!!! ROUTINE [%s] - SHARED VARIABLE [%I64d].\n",FunctionName,FastMutexLocalObjectSharedVariable));
			}
			while(false);
