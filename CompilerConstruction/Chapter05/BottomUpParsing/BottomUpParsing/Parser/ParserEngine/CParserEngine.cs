...
//-------------------------------------------------------------------------------------------------------
		...
		private void CalculcateDFA(bool PrintLog)
		{
			...
			if (PrintLog==true)
			{
				Console.WriteLine(string.Format("Number of LR(1) DFA STATES: [{0}] !",MDFAStateCollectionLR1.DFAStates.Length));
			}
		}
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
		...
		public bool DoParseLR1OrLALR(bool PrintLog, EParserType ParserType, CToken[] SourceCodeTokens, out CParsingStackItem RootParsingStackItem, CParsingErrorCollection ParsingErrorCollection, bool DoErrorRecovery)
		{
			...
			CParsingStackItem										TopParsingStackItem=new CParsingStackItem(StartTreeNode,StartDFAState);
			...
			while(true)
			{
				// Urobim PEEK nad SOURCE CODE INPUT STREAM.
						...
						if (ErrorParsingTableRow.HasGotoCells()==true)
						{
							...
							while(true)
							{
								...
								if (GotoCellWithLegalAction!=null)
								{
									...
									CParsingStackItem				NewParsingStackItem=new CParsingStackItem(NewParseTreeNode,NewDFAState);
				...
				else if ((ParsingTableOperation is CParsingTableOperation_Reduce)==true)
				{
					...
					CParsingStackItem								NewItem=new CParsingStackItem(ChoiceTreeNode,GotoOperation.GotoDFAState);
