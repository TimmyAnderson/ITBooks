...
{
//----------------------------------------------------------------------------------------------------------------------
	public static class CCookieCounterCountEndpoint
	{
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
		public static async Task Endpoint(HttpContext Context)
		{
			...
			if (Context.Request.Cookies.TryGetValue("COUNTER_1",out StringCounter1)==true)
			{
				int.TryParse(StringCounter1,out Counter1);
			}

			// !!! Nacita sa COOKIE VALUE.
			if (Context.Request.Cookies.TryGetValue("COUNTER_2",out StringCounter2)==true)
			{
			...
			Context.Response.Cookies.Append("COUNTER_1",Counter1.ToString(),Counter1Options);

			// !!! COOKIE VALUE sa ulozi do COOKIES HTTP RESPONSE MESSAGE.
			Context.Response.Cookies.Append("COUNTER_2",Counter2.ToString(),Counter2Options);
			...
			Content.Append($" COUNTER 1 [{Counter1}] will EXPIRE at [{CreateTime(Cookie1ExpirationTime)}] !");
			Content.Append($"<br/>");
			Content.Append($" COUNTER 2 [{Counter2}] will EXPIRE at [{CreateTime(Cookie2ExpirationTime)}] !");
			Content.Append($"<br/>");
			Content.Append($"<a href=\"/CookieCounterCount\">Reload PAGE !</a>");
			Content.Append($"<br/>");
			Content.Append($"<a href=\"/CookieCounterDelete\">Delete COOKIES !</a>");
