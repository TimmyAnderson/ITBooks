...
void IRPCompleteSimulatorReadIRP(PVOID Parameter)
{
	...
	if (DataToRead!=NULL)
	{
		...
		if (DataLengthToRead>=DATA_LENGTH)
		{
			for(;;)
			{
				...
				if (NT_SUCCESS(Status=RtlStringCbCopyA(&DataToRead[NumberOfBytesWritten],DATA_LENGTH-NumberOfBytesWritten,Prefix))==false)
				{
					DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): RtlStringCbCopyA() FAILED [%X] !\n",Status);
					HasError=true;
					break;
				}
				...
				for(int Index=0;Index<NUMBER_OF_ITERATIONS;Index++)
				{
					...
					if (NT_SUCCESS(Status=RtlStringCbPrintfA(LocalBuffer,sizeof(LocalBuffer),"%d",Index+1))==false)
					{
					...
					if (NT_SUCCESS(Status=RtlStringCbLengthA(LocalBuffer,sizeof(LocalBuffer),&LocalBufferSize))==false)
					{
					...
					if (Index>0)
					{
						...
						if (NT_SUCCESS(Status=RtlStringCbCopyA(&DataToRead[NumberOfBytesWritten],DATA_LENGTH-NumberOfBytesWritten,Separator))==false)
						{
							DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): RtlStringCbCopyA() FAILED [%X] !\n",Status);
							HasError=true;
							break;
					...
					if (NT_SUCCESS(Status=RtlStringCbCopyA(&DataToRead[NumberOfBytesWritten],DATA_LENGTH-NumberOfBytesWritten,LocalBuffer))==false)
					{
						DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): RtlStringCbCopyA() FAILED [%X] !\n",Status);
						HasError=true;
						break;
				...
				if (HasError==false)
				{
					...
					if (NT_SUCCESS(Status=RtlStringCbCopyA(&DataToRead[NumberOfBytesWritten],DATA_LENGTH-NumberOfBytesWritten,Postfix))==false)
					{
						DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): RtlStringCbCopyA() FAILED [%X] !\n",Status);
						HasError=true;
						break;
	...
	/*
	// !!!!! MANUALNE UVOLNENIE MDL NIE JE NUTNE, pretoze MDL uvolnuje WINDOWS AUTOMATICKY pri volani FUNCTION IoCompleteRequest(). DRIVER vsak fungoval KOREKTNE, aj ked sa MDL uvolnilo nasledujucim CODE.
	// !!!!! Uvolnenie PAGES a MDL sa MUSI vykonat PRED volanim FUNCTION IoCompleteRequest(), pretoze FUNCTION IoCompleteRequest() MDL uvolnuje AUTOMATICKY.
	MmUnlockPages(CurrentMDL);
	IoFreeMdl(CurrentMDL);
	// !!! Ak sa FIELD [MdlAddress] NENASTAVIL na NULL, WINDOWS hodil BUG CHECK.
	InterruptRequestPacket->MdlAddress=NULL;
	*/
...
// !!!!! Tento CODE NIE JE CELKOM KOREKTNY, pretoze v pripade, ze pocas behu THREADU dojde k odstraneniu DRIVER z MEMORY WINDOWS detekuje, ze THREAD vytvoreny DRIVER nebol odstraneny z MEMORY a sposobi BUG CHECK [0xCE] [DRIVER_UNLOADED_WITHOUT_CANCELLING_PENDING_OPERATIONS]. THREADS by sa mali vytvarat pri starte DRIVER a uvolnovat pri ukonceni DRIVER.
_Function_class_(KSTART_ROUTINE)
void IRPCompleteSimulatorWriteIRP(PVOID Parameter)
{
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): WORKER THREAD STARTED !\n");
	...
	ULONG														DataLengthToWrite;

	if (DataToWrite!=NULL)
	{
		DataLengthToWrite=MmGetMdlByteCount(CurrentMDL);

		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): !!!!!!!!!! DATA LENGTH [%d] !\n",DataLengthToWrite);

		ULONG													NumberOfFragments=((DataLengthToWrite % FRAGMENT_SIZE)==0) ? (DataLengthToWrite/FRAGMENT_SIZE) : ((DataLengthToWrite/FRAGMENT_SIZE)+1);
		...
		for(ULONG Index=0;Index<NumberOfFragments;Index++)
		{
			...
			{
				FragmentSize=DataLengthToWrite-(Index * FRAGMENT_SIZE);
			}

			PCSTR												StartIndex=&DataToWrite[Index*FRAGMENT_SIZE];
...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	for(;;)
	{
		...
		// !!!!! Kedze DRIVER nenastavuje ani IO, ani DIRECT MODE, tak za pouzije NO MODE.
		...
		Status=IoRegisterDeviceInterface(PhysicalDeviceObject,&GUID_DEVINTERFACE_NO_MODE_DRIVER,NULL,&DeviceExtensionData->InterfaceName);
...
NTSTATUS DispatchRead(PDEVICE_OBJECT DeviceObject, PIRP InterruptRequestPacket)
{
	...
	ULONG														DataLengthToRead=StackLocation->Parameters.Read.Length;
	PMDL														MDL=NULL;
	bool														PagesLocked=false;

	__try
	{
		MDL=IoAllocateMdl(InterruptRequestPacket->UserBuffer,DataLengthToRead,FALSE,FALSE,InterruptRequestPacket);

		if (MDL!=NULL)
		{
			// !!!!! Kedze FUNCTION MmProbeAndLockPages() moze hodit EXCEPTION je NEVYHNUTNE obalit volanie tejto FUNCTION v TRY CATCH CLAUSES.
			...
			if (ThreadParameters!=NULL)
			{
				...
				if (NT_SUCCESS(Status=PsCreateSystemThread(&ThreadHandle,THREAD_ALL_ACCESS,&ThreadHandleObjectAttributes,NULL,NULL,IRPCompleteSimulatorReadIRP,ThreadParameters))==true)
				{
			...
			DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MmProbeAndLockPages() FAILED !\n");
		}
		else
		{
...
NTSTATUS DispatchWrite(PDEVICE_OBJECT DeviceObject, PIRP InterruptRequestPacket)
{
	...
	PIO_STACK_LOCATION											StackLocation=IoGetCurrentIrpStackLocation(InterruptRequestPacket);
	ULONG														DataLengthToWrite=StackLocation->Parameters.Write.Length;
	PMDL														MDL=NULL;
	bool														PagesLocked=false;

	__try
	{
		MDL=IoAllocateMdl(InterruptRequestPacket->UserBuffer,DataLengthToWrite,FALSE,FALSE,InterruptRequestPacket);
		...
		if (MDL!=NULL)
		{
			...
			if (ThreadParameters!=NULL)
			{
				...
				// !!!!! V THREADE sa SIMULUJE PROCESSING IRP.
				if (NT_SUCCESS(Status=PsCreateSystemThread(&ThreadHandle,THREAD_ALL_ACCESS,&ThreadHandleObjectAttributes,NULL,NULL,IRPCompleteSimulatorWriteIRP,ThreadParameters))==true)
				{
					DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): SYSTEM THREAD CREATED !\n");
