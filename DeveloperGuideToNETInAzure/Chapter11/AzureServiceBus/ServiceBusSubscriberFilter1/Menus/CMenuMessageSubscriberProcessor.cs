...
{
//--------------------------------------------------------------------------------------------------------------------------------
	public sealed class CMenuMessageSubscriberProcessor : CMenu
	{
//--------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------
		public CMenuMessageSubscriberProcessor()
...
//--------------------------------------------------------------------------------------------------------------------------------
		private Task MMessageProcessorProcessMessageAsync(ProcessMessageEventArgs Args)
		{
			...
			string												ReplyTo=ReceivedMessage?.ReplyTo ?? "";

			byte[]												BodyRaw=ReceivedMessage?.Body?.ToArray();
			string												BodyString=(BodyRaw!=null) ? Encoding.UTF8.GetString(BodyRaw) : "";

			StringBuilder										SB=new StringBuilder();
			IReadOnlyDictionary<string,object>					ApplicationProperties=ReceivedMessage.ApplicationProperties;
			...
			Console.WriteLine($"MESSAGE DETECTED. IDENTIFIER [{Identifier}] ENTITY PATH [{EntityPath}] FULLY QUALIFIED NAMESPACE [{FullyQualifiedNamespace}] MESSAGE ID [{MessageID}] CONTENT TYPE [{ContentType}] TO [{To}] REPLY TO [{ReplyTo}] BODY [{BodyString}].{Properties}");
...
//--------------------------------------------------------------------------------------------------------------------------------
		private void ExecuteCommandStartReceivingMessages(string CommandID, object[] Parameters)
		{
			...
			try
			{
				...
				MMessageProcessor=MClient.CreateProcessor(CConstants.SERVICE_BUS_TOPIC_NAME,CConstants.SERVICE_BUS_SUBSCRIPTION_FILTER_1_NAME);
