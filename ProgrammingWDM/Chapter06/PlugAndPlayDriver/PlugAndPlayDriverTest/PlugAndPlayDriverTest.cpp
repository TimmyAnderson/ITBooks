...
void Test4(void)
{
	...
	if (Handle!=INVALID_HANDLE_VALUE)
	{
		...
		if (DeviceIoControl(Handle,IOCTL_SYNCHRONOUS_CUSTOM_NOTIFICATION,NULL,0,NULL,0,&BytesReturned,NULL)!=FALSE)
		{
			wprintf_s(L"IO CONTROL SUCCESSFULLY SENT - BytesReturned [%d] !\n",BytesReturned);
		}
		else
		{
			wprintf_s(L"Can't SEND IO CONTROL - ERROR [%d] !\n",GetLastError());
...
void Test5(void)
{
	...
	if (Handle!=INVALID_HANDLE_VALUE)
	{
		...
		if (DeviceIoControl(Handle,IOCTL_ASYNCHRONOUS_CUSTOM_NOTIFICATION,NULL,0,NULL,0,&BytesReturned,NULL)!=FALSE)
		{
			wprintf_s(L"IO CONTROL SUCCESSFULLY SENT - BytesReturned [%d] !\n",BytesReturned);
		}
		else
