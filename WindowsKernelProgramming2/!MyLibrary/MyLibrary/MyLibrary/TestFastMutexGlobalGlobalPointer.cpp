...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
ULONG64															FastMutexGlobalGlobalPointerSharedVariable=0;
CFastMutexGlobal*												FastMutexGlobalGlobalPointer=nullptr;
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexGlobalGlobalPointer1(PVOID StartContext)
{
	...
	for(LONG64 Index=0;Index<LONG64(1000*1000);Index++)
	{
		...
		{
			FastMutexGlobalGlobalPointerSharedVariable++;
		}
	}
...
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexGlobalGlobalPointer2(PVOID StartContext)
{
	const char*													FunctionName=__FUNCTION__;
...
NTSTATUS TestFastMutexGlobalGlobalPointer(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
	...
	{
		FastMutexGlobalGlobalPointer=new (EPoolFlags::E_POOL_FLAG_PAGED) CFastMutexGlobal();

		if (FastMutexGlobalGlobalPointer!=nullptr)
		{
			FastMutexGlobalGlobalPointer->Init();

			{
				...
				do
				{
					Status=CHelpers::CreateThread(DeviceObject,"111",ThreadFastMutexGlobalGlobalPointer1,FastMutexGlobalGlobalPointer,&ThreadObject1);
					...
					Status=CHelpers::CreateThread(DeviceObject,"222",ThreadFastMutexGlobalGlobalPointer2,FastMutexGlobalGlobalPointer,&ThreadObject2);
					...
					KdPrint(("!!!!!!!!!! ROUTINE [%s] - SHARED VARIABLE [%I64d].\n",FunctionName,FastMutexGlobalGlobalPointerSharedVariable));
				}
				while(false);
			...
			delete(FastMutexGlobalGlobalPointer);
			FastMutexGlobalGlobalPointer=nullptr;
		}
		else
		{
