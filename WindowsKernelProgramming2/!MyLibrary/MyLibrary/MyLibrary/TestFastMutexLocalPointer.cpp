...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
ULONG64															FastMutexLocalPointerSharedVariable=0;
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexLocalPointer1(PVOID StartContext)
{
	...
	for(LONG64 Index=0;Index<LONG64(1000*1000);Index++)
	{
		...
		{
			FastMutexLocalPointerSharedVariable++;
		}
	}
...
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexLocalPointer2(PVOID StartContext)
{
	const char*													FunctionName=__FUNCTION__;
...
NTSTATUS TestFastMutexLocalPointer(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
	...
	{
		CFastMutex*												FastMutexLocalPointer=new (EPoolFlags::E_POOL_FLAG_PAGED) CFastMutex();

		if (FastMutexLocalPointer!=nullptr)
		{
			{
				...
				do
				{
					Status=CHelpers::CreateThread(DeviceObject,"111",ThreadFastMutexLocalPointer1,FastMutexLocalPointer,&ThreadObject1);
					...
					Status=CHelpers::CreateThread(DeviceObject,"222",ThreadFastMutexLocalPointer2,FastMutexLocalPointer,&ThreadObject2);
					...
					KdPrint(("!!!!!!!!!! ROUTINE [%s] - SHARED VARIABLE [%I64d].\n",FunctionName,FastMutexLocalPointerSharedVariable));
				}
				while(false);
			...
			delete(FastMutexLocalPointer);
			FastMutexLocalPointer=nullptr;
		}
		else
		{
