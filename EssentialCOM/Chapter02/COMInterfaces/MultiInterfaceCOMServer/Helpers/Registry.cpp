...
HRESULT RegisterServer(const wchar_t* ModuleName, REFCLSID CLSID, const wchar_t* FriendlyName, const wchar_t* VerIndProgID, const wchar_t* ProgID, const wchar_t* ThreadingModel)
{
	...
	else
	{
		...
		// Zisti nazov .EXE.
		GetModuleFileName(HandleModule,EXEPath,sizeof(EXEPath));

		wchar_t*												Position=wcsstr(EXEPath,L".exe");
		...
		// Zmeni .EXE za .TLB. Predpoklada sa, ze DLL a TLB su v TOM ISTOM DIRECTORY.
		Position[1]='t';
		Position[2]='l';
		Position[3]='b';
		...
		// !!! Zaregistruje TYPE LIBRARY.
		if (SUCCEEDED(HR=LoadTypeLibEx(EXEPath,REGKIND_REGISTER,&TypeLib)))
		{
			TypeLib->Release();
		}
	}
...
LONG UnregisterServer(REFCLSID CLSID, const wchar_t* VerIndProgID, const wchar_t* ProgID, REFGUID LIBID)
{
	...
	// !!!!! Ak ma MIDL nastaveny 'Target Environment' na X64, MUSI byt posledny parameter nastaveny na 'SYS_WIN64'.
