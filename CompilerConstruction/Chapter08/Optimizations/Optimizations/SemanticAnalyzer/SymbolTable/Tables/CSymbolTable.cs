...
//-------------------------------------------------------------------------------------------------------
		public int												SymbolItemsStackSize
		{
			get
			{
				...
				{
					if ((Item.SymbolTableData is CSymbolTableData_Variable)==true)
					{
						CSymbolTableData_Variable				Variable=(CSymbolTableData_Variable) Item.SymbolTableData;

						// Ak je premenna v ECX, tak nie je na STACK.
						if (Variable.IsECXRegisterOccupied==false)
						{
...
//-------------------------------------------------------------------------------------------------------
		public void AddSymbol(CSymbolTableData Data)
		{
			...
			// !!! Aj na urovni DEKLARACII FUNKCIE je OFFSET 0, pretoze OFFSET sa vypocita az ked su VSETKY PREMENNE zaradene do SYMBOL TABLE a je jasne kolko parametrov ma dana funkcia.
			...
			{
				// Optimalizacia presuvajuca VARIABLE typu INT do registra ECX.
				if ((Data is CSymbolTableData_Variable)==true)
				{
					CSymbolTableData_Variable					TypedData=(CSymbolTableData_Variable) Data;

					// Iba INT premenne mozem ulozit do ECX.
					if (TypedData.SymbolType==ESymbolType.E_ST_INT)
					{
						bool									IsECXRegisterOccupied=false;
						CSymbolTable							CurrentSymbolTable=this;

						// Zistim ci premenna typu INT moze byt ECX - teda ci ECX uz nie je rezerovany pre inu premennu.
						while(true)
						{
							if (CurrentSymbolTable==null || (CurrentSymbolTable is CSymbolTable_Root)==true)
							{
								break;
							}

							CSymbolTableItem[]					SymbolTableItems=CurrentSymbolTable.MSymbolItems.ToArray();

							foreach(CSymbolTableItem SymbolTableItem in SymbolTableItems)
							{
								CSymbolTableData_Variable		Variable=SymbolTableItem.SymbolTableData as CSymbolTableData_Variable;

								if (Variable!=null)
								{
									if (Variable.IsECXRegisterOccupied==true)
									{
										IsECXRegisterOccupied=true;
										break;
									}
								}
							}

							CurrentSymbolTable=CurrentSymbolTable.MParentScopeTable;
						}

						if (IsECXRegisterOccupied==false)
						{
							// Premenna bude v registry ECX.
							TypedData.IsECXRegisterOccupied=true;
						}
					}

					if (TypedData.IsECXRegisterOccupied==true)
					{
						// Kedze premenna bude v ECX, tak moze do StackOffset dat 0.
						StackOffset=0;
					}
					else
					{
