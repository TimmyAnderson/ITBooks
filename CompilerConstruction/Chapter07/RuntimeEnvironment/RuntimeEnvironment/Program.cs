...
//-------------------------------------------------------------------------------------------------------
		private static void PrintSymbolTable(CSymbolTable SymbolTable)
		{
			int				Padding=SymbolTable.SymbolTableLevel*3;

			Console.WriteLine(string.Format("{0}{1}, StackSize: [{2}]","".PadRight(Padding,' '),SymbolTable.ToString(),SymbolTable.SymbolItemsStackSize));

			foreach(CSymbolTableItem Item in SymbolTable.SymbolItems)
			{
				CSymbolTableData			Data=Item.SymbolTableData;

				Console.WriteLine(string.Format("{0}{1} {2} - SIZE: {3}, OFFSET: {4}","".PadRight(Padding+1,' '),Data.SymbolName,Data.SymbolType,Data.GetSizeof(),Item.StackOffset));
			}
...
//-------------------------------------------------------------------------------------------------------
		private static void DoSemanticAnalysis(string ParserRulesFile, CParseTreeNode ParseTreeRoot, bool PrintLog)
		{
			...
			CSymbolTable						SymbolTableRoot=SemanticAnalyzer.DoAnalysis(NonterminalCollection,ParseTreeRoot,SemanticErrors,PrintLog);
			...
			else
			{
				...
				SymbolTableRoot.TraverseSymbolTableTree(PrintSymbolTable);
			}
		}
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
