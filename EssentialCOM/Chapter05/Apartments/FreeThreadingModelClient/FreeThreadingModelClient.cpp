...
#include <crtdbg.h>
#include <process.h>
#include "..\FreeThreadingModelServer\FreeThreadingModelServer_h.h"
...
struct SThreadParams
{
	...
	IFreeThreadingModelTest*									MCOMPointer;
};
//------------------------------------------------------------------------------------------------------
...
unsigned int __stdcall ThreadNoMarshalingTestCOM1(void* Param)
{
...

	{
		...
		CSmartCOMPointer<IFreeThreadingModelTest>				Interface=(IFreeThreadingModelTest*) TypedParam->MCOMPointer;
...
unsigned int __stdcall ThreadNoMarshalingTestCOM2(void* Param)
{
	...
	{
		// !!!!! CHYBA !!! PROXY na COM OBJECT (BEZ VYKONANIA MARSHALINGU) prenasam do THREADU s NEKOPMATIBILNYM APARTMENT (MTA).
		CSmartCOMPointer<IFreeThreadingModelTest>				Interface=(IFreeThreadingModelTest*) TypedParam->MCOMPointer;
...
unsigned int __stdcall ThreadMarshalingTestCOM1(void* Param)
{
...

	{
		...
		{
			CSmartCOMPointer<IFreeThreadingModelTest>			Interface=NULL;

			// !!!!! OK !!! Kedze APARTMENTS NIE SU KOMPATIBILNE (STA - MAIN THREAD, STA - WORKER THREAD, MTA - COM OBJECT), vytvara sa PROXY na COM OBJECT.
			if (SUCCEEDED(HR=CoUnmarshalInterface(Stream.GetInterface(),__uuidof(IFreeThreadingModelTest),(void**) &Interface)))
			{
...
unsigned int __stdcall ThreadMarshalingTestCOM2(void* Param)
{
...

	{
		...
		if (SUCCEEDED(HR=CreateStreamOnHGlobal(TypedParam->MByteStream,FALSE,&Stream)))
		{
			...
			// !!!!! OK !!! Kedze APARTMENTS NIE SU KOMPATIBILNE (STA - MAIN THREAD, MTA - WORKER THREAD, MTA - COM OBJECT), vytvara sa PROXY na COM OBJECT.
			if (SUCCEEDED(HR=CoUnmarshalInterface(Stream.GetInterface(),__uuidof(IFreeThreadingModelTest),(void**) &Interface)))
			{
				wprintf_s(L"THREAD - POINTER [%p] !\n",Interface.GetInterface());
...
unsigned int __stdcall InterThreadMarshalingTestCOM1(void* Param)
{
...

	{
		...
		// !!!!! OK !!! Kedze APARTMENTS NIE SU KOMPATIBILNE (STA - MAIN THREAD, STA - WORKER THREAD, MTA - COM OBJECT), vytvara sa PROXY na COM OBJECT.
		if (SUCCEEDED(HR=CoGetInterfaceAndReleaseStream(TypedParam->MStream,__uuidof(IFreeThreadingModelTest),(void**) &Interface)))
		{
			wprintf_s(L"THREAD - POINTER [%p] !\n",Interface.GetInterface());
...
void NoMarshalingTestCOM1(void)
{
	...
	try
	{
		...
		HRESULT													HR=CoCreateInstance(CLSID_FreeThreadingModelObject,NULL,CLSCTX_INPROC_SERVER,__uuidof(IFreeThreadingModelTest),(void**) &Interface);
...
void MarshalingTestCOM1(void)
{
	...
	// 2. Vytvori SINGLE-THREAD APARTMENT.
	// 3. Vytvoria sa SINGLE-THREAD APARTMENT WORKER THREADS.
	// !!!!! 4. Kedze COM OBJECT bol vytvoreny v MTA (COM OBJECT je MTA a teda VZDY je vytvoreny v MTA) MESSAGE PUMP NIE JE POTREBNA.
	...
	try
	{
		...
		if (SUCCEEDED(HR))
		{
			...
			for (int Index=0;Index<NUMBER_OF_THREADS;Index++)
			{
				...
				{
					if (SUCCEEDED(CoMarshalInterface(Stream.GetInterface(),__uuidof(IFreeThreadingModelTest),Interface.GetInterface(),MSHCTX_DIFFERENTMACHINE,NULL,MSHLFLAGS_NORMAL)))
					{
	...
	try
	{
		...
		if (SUCCEEDED(HR))
		{
			...
			for (int Index=0;Index<NUMBER_OF_THREADS;Index++)
			{
				...
				if (SUCCEEDED(CoMarshalInterThreadInterfaceInStream(__uuidof(IFreeThreadingModelTest),Interface.GetInterface(),&ThreadParams[Index].MStream)))
				{
