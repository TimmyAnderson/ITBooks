...
void IRPCompleteSimulatorReadIRP(PVOID Parameter)
{
	...
	// !!! V BUFFERED MODE FIELD [IO_STACK_LOCATION.Parameters.Read.Length] urcuje pocet BYTES, ktore ma DRIVER nacitat.
	ULONG														DataLengthToRead=StackLocation->Parameters.Read.Length;
	// !!! V BUFFERED MODE FIELD [IRP.AssociatedIrp.SystemBuffer] odkazuje na BUFFER, do ktoreho sa maju zapisat data nacitane z HW DEVICE.
	PSTR														DataToRead=(PSTR) InterruptRequestPacket->AssociatedIrp.SystemBuffer;
...
void IRPCompleteSimulatorWriteIRP(PVOID Parameter)
{
	...
	PIO_STACK_LOCATION											StackLocation=IoGetCurrentIrpStackLocation(InterruptRequestPacket);
	// !!! V BUFFERED MODE FIELD [IO_STACK_LOCATION.Parameters.Write.Length] urcuje pocet BYTES, ktore ma DRIVER zapisat.
	ULONG														DataLengthToWrite=StackLocation->Parameters.Write.Length;
	// !!! V BUFFERED MODE FIELD [IRP.AssociatedIrp.SystemBuffer] odkazuje na BUFFER, z ktoreho sa maju data zapisat do HW DEVICE.
	PCSTR														DataToWrite=(PCSTR) InterruptRequestPacket->AssociatedIrp.SystemBuffer;
...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	for(;;)
	{
		...
		// !!!!! DRIVER definuje, ze sa bude pouzivat BUFFERED MODE.
		...
		Status=IoRegisterDeviceInterface(PhysicalDeviceObject,&GUID_DEVINTERFACE_BUFFERED_MODE_DRIVER,NULL,&DeviceExtensionData->InterfaceName);
