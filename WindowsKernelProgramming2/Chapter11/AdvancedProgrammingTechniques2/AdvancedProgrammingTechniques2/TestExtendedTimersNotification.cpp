...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ExtendedTimersNotificationTimerCallback(PEX_TIMER Timer, PVOID Context)
{
...
//----------------------------------------------------------------------------------------------------------------------
void ExtendedTimersNotificationTimerDeleteCallback(PVOID Context)
{
	// !!! CODE sa vykonava v IRQL [DISPATCH_LEVEL].
...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ExtendedTimersNotificationThread1(PVOID StartContext)
{
...
//----------------------------------------------------------------------------------------------------------------------
void ExtendedTimersNotificationThread2(PVOID StartContext)
{
	PEX_TIMER													Timer=static_cast<PEX_TIMER>(StartContext);
...
//----------------------------------------------------------------------------------------------------------------------
void ExtendedTimersNotificationThread3(PVOID StartContext)
{
	PEX_TIMER													Timer=static_cast<PEX_TIMER>(StartContext);
...
NTSTATUS TestExtendedTimersNotification(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
...

	{
		...
		// !!! EXTENDED TIMER je NOTIFICATION TIMER, lebo je nastaveny FLAG [EX_TIMER_NOTIFICATION].
		PEX_TIMER												Timer=ExAllocateTimer(ExtendedTimersNotificationTimerCallback,nullptr,EX_TIMER_HIGH_RESOLUTION | EX_TIMER_NOTIFICATION);
		...
		if (Timer!=nullptr)
		{
			...
			do
			{
				Status=CHelpers::CreateThread(DeviceObject,"111",ExtendedTimersNotificationThread1,Timer,&ThreadObject1);
				...
				Status=CHelpers::CreateThread(DeviceObject,"222",ExtendedTimersNotificationThread2,Timer,&ThreadObject2);
				...
				Status=CHelpers::CreateThread(DeviceObject,"333",ExtendedTimersNotificationThread3,Timer,&ThreadObject3);
			...
			DeleteParameters.DeleteCallback=ExtendedTimersNotificationTimerDeleteCallback;
