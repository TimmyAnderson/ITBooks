...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	for(;;)
	{
		...
		Status=IoRegisterDeviceInterface(PhysicalDeviceObject,&GUID_DEVINTERFACE_PORT_RESOURCES_DRIVER,NULL,&DeviceExtensionData->InterfaceName);
...
NTSTATUS __stdcall StartDevice(PDEVICE_OBJECT DeviceObject, PCM_PARTIAL_RESOURCE_LIST RawList, PCM_PARTIAL_RESOURCE_LIST TranslatedList)
{
	...
	for (ULONG Index=0;Index<NumberOfResources;++Index,++Resource)
	{
		...
		switch(Resource->Type)
		{
			...
			{
				Address=Resource->u.Port.Start;

				DeviceExtensionData->NumberOfPorts=Resource->u.Port.Length;
				// !!! Ak FLAG [CM_RESOURCE_PORT_IO] NIE JE NASTAVENY, tak CPU NEPODPORUJE PORT MAPPED IO a ADDRESS musi byt MAPPED na MEMORY ADDRESS.
				DeviceExtensionData->ArePortsMapped=((Resource->Flags & CM_RESOURCE_PORT_IO)==0 ? TRUE : FALSE);

				DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): PORT ASSIGNED - PORT BASE ADDRESS [%I64d], NUMBER OF PORTS [%d], ARE PORTS MAPPED [%d] !\n",Address.QuadPart,DeviceExtensionData->NumberOfPorts,DeviceExtensionData->ArePortsMapped);
			}
			break;
	...
	if (Address.QuadPart!=NULL && DeviceExtensionData->ArePortsMapped!=FALSE)
	{
		// !!! Kedze CPU ARCHITECTURE IO PORT NEPODPORUJE, je NUTNE vykonat MAPPING na ADDRESSES v MAIN MEMORY.
		DeviceExtensionData->PortBaseAddress=(PUCHAR) MmMapIoSpace(Address,DeviceExtensionData->NumberOfPorts,MmNonCached);

		if (DeviceExtensionData->PortBaseAddress==NULL)
		{
	...
	{
		// !!! Kedze CPU ARCHITECTURE IO PORT PODPORUJE, nie je treba vykonat ziaden MAPPING.
		DeviceExtensionData->PortBaseAddress=(PUCHAR) Address.QuadPart;
	}

	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): PORT ASSIGNED - PORT BASE ADDRESS [%p], NUMBER OF PORTS [%d], ARE PORTS MAPPED [%d] !\n",DeviceExtensionData->PortBaseAddress,DeviceExtensionData->NumberOfPorts,DeviceExtensionData->ArePortsMapped);
...
VOID __stdcall StopDevice(IN PDEVICE_OBJECT DeviceObject, BOOLEAN)
{
	...
	// !!! Ak je IO PORT MAPPED do MAIN MEMORY, musi byt uvolneny.
	if (DeviceExtensionData->PortBaseAddress!=NULL && DeviceExtensionData->ArePortsMapped!=FALSE)
	{
		MmUnmapIoSpace(DeviceExtensionData->PortBaseAddress,DeviceExtensionData->NumberOfPorts);

		DeviceExtensionData->PortBaseAddress=NULL;
	}
