...
//----------------------------------------------------------------------------------------------------------------------
		...
		public CDBContext CreateDbContext(string[] Args)
		{
			...
			EMigrationType										MigrationType=EMigrationType.E_MIGRATION_VERSION_2;

			if (Args.Length>0)
			{
				string											Parameter=Args[0];

				if (Parameter=="USE_MIGRATION_VERSION_1")
				{
					MigrationType=EMigrationType.E_MIGRATION_VERSION_1;
				}
				else if (Parameter=="USE_MIGRATION_VERSION_2")
				{
					MigrationType=EMigrationType.E_MIGRATION_VERSION_2;
				}
				else
				{
					throw(new InvalidOperationException($"UNSUPPORTED VALUE [{Parameter}]."));
				}
			}

			// !!! Vytvori sa instancia DB CONTEXT CLASS.
			CDBContext											DBContext=new CDBContext(MigrationType);
