...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	for(;;)
	{
		...
		Status=IoRegisterDeviceInterface(PhysicalDeviceObject,&GUID_DEVINTERFACE_MEMORY_RESOURCES_DRIVER,NULL,&DeviceExtensionData->InterfaceName);
...
NTSTATUS __stdcall StartDevice(PDEVICE_OBJECT DeviceObject, PCM_PARTIAL_RESOURCE_LIST RawList, PCM_PARTIAL_RESOURCE_LIST TranslatedList)
{
	...
	for (ULONG Index=0;Index<NumberOfResources;++Index,++Resource)
	{
		...
		switch(Resource->Type)
		{
			...
			{
				PhysicalAddress=Resource->u.Port.Start;

				DeviceExtensionData->NumberOfAddresses=Resource->u.Port.Length;

				DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MEMORY ADDRESS ASSIGNED - MEMORY BASE ADDRESS [%I64d], NUMBER OF ADDRESSES [%d] !\n",PhysicalAddress.QuadPart,DeviceExtensionData->NumberOfAddresses);
			}
			break;
	...
	if (PhysicalAddress.QuadPart!=NULL)
	{
		// !!! Pri MEMORY MAPPED IO je NEVYHNUTNE vykonat konverziu z PHYSICAL ADDRESS na VIRTUAL ADDRESS.
		DeviceExtensionData->MemoryBaseAddress=(PUCHAR) MmMapIoSpace(PhysicalAddress,DeviceExtensionData->NumberOfAddresses,MmNonCached);

		if (DeviceExtensionData->MemoryBaseAddress==NULL)
		{
			return(STATUS_NO_MEMORY);
		}
	}

	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MEMORY ADDRESS ASSIGNED - MEMORY BASE ADDRESS [%p], NUMBER OF ADDRESSES [%d] !\n",DeviceExtensionData->MemoryBaseAddress,DeviceExtensionData->NumberOfAddresses);
...
VOID __stdcall StopDevice(IN PDEVICE_OBJECT DeviceObject, BOOLEAN)
{
	...
	// !!! Ak sa pouziva MEMORY MAPPING IO, je NUTNE uvolnit VIRTUAL ADDRESS cez ktoru sa MEMORY MAPPING IO realizuje.
	if (DeviceExtensionData->MemoryBaseAddress!=NULL)
	{
		MmUnmapIoSpace(DeviceExtensionData->MemoryBaseAddress,DeviceExtensionData->NumberOfAddresses);

		DeviceExtensionData->MemoryBaseAddress=NULL;
	}
