						...
						while(SymbolIndex<SymbolsLenth)
						{
							...
							if ((SymbolXi is CNonterminal)==true)
							{
								...
								else
								{
									...
									{
										FirstSetSymbols[Index]=Symbols[Index+SymbolIndexXiPlus1];
									}
...
//-------------------------------------------------------------------------------------------------------
		...
		private void CalculcateNFA()
		{
			...
			MLog.AppendLine(string.Format("Number of NFA STATES: [{0}] !",MNFAStateCollection.NFAStates.Length));
		}
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
		...
		private void CalculcateDFA()
		{
			...
			MLog.AppendLine(string.Format("Number of DFA STATES: [{0}] !",MDFAStateCollectionLR1.DFAStates.Length));
		}
//-------------------------------------------------------------------------------------------------------
		// Vypocitava LALR(1) DFA z DFA pre LR(1).
		private void CalculateLALRDFA()
		{
			...
			while(DFAStates.Count>0)
			{
				...
				for(int Index=0;Index<DFAStates.Count;Index++)
				{
					if (DFAStates[Index].ContainsSameLALRCore(LALRCore)==true)
					{
						StatesWithSameLALRCore.Add(DFAStates[Index]);
						DFAStates.RemoveAt(Index);
						Index--;
					}
				}
			...
			MLog.AppendLine(string.Format("Number of LALR(1) DFA STATES: [{0}] !",MDFAStateCollectionLALR1.DFAStates.Length));
		}
//-------------------------------------------------------------------------------------------------------
		// Vypocitava PARSING TABLE z DFA.
		private void CalculcateParsingTable()
		{
			MLog.AppendLine("CALCULATION of DFA and PARSING TABLE for LR(1) !");

			CalculcateDFA();

			MParsingTableLR1.InitializeParsingTable(MDFAStateCollectionLR1,MNonterminalCollection,MTerminalCollection,MAugmentedNonterminal);

			MLog.AppendLine(string.Format("LR(1) - Number of SHIFT-REDUCE conflicts: [{0}] !",MParsingTableLR1.NumberOfShiftReduceConflicts));
			MLog.AppendLine(string.Format("LR(1) - Number of REDUCE-REDUCE conflicts: [{0}] !",MParsingTableLR1.NumberOfReduceReduceConflicts));

			MLog.AppendLine("CALCULATION of DFA and PARSING TABLE for LALR(1) !");

			CalculateLALRDFA();

			MParsingTableLALR1.InitializeParsingTable(MDFAStateCollectionLALR1,MNonterminalCollection,MTerminalCollection,MAugmentedNonterminal);

			MLog.AppendLine(string.Format("LALR(1) - Number of SHIFT-REDUCE conflicts: [{0}] !",MParsingTableLALR1.NumberOfShiftReduceConflicts));
			MLog.AppendLine(string.Format("LALR(1) - Number of REDUCE-REDUCE conflicts: [{0}] !",MParsingTableLALR1.NumberOfReduceReduceConflicts));
		}
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
		...
		public bool Parse(EParserType ParserType, CTokenCollection TokenCollection, out CParsingStackItem RootParsingStackItem, CCompilerErrorCollection Errors, bool DoErrorRecovery)
		{
			MLog.Clear();

			CalculcateParsingTable();
			...
			foreach(CToken SourceCodeToken in TokenCollection.TokensForParsing)
			{
			...
			while(true)
			{
				...
				{
					CParsingError								ParsingError=new CParsingError("END of SOURCE CODE REACHED !",(CurrentSourceCodeItem!=null) ? CurrentSourceCodeItem.Terminal : null,CurrentItem,MSourceTerminalsQueue.GetTokenOnTop().SourceCodeIndex);

					Errors.AddError(ParsingError);
					RootParsingStackItem=null;

					MLog.AppendLine(string.Format("NumberOfCycles: [{0}] !",NumberOfCycles));
					MLog.AppendLine(string.Format("NumberOfShifts: [{0}] !",NumberOfShifts));
					MLog.AppendLine(string.Format("NumberOfReduces: [{0}] !",NumberOfReduces));
				...
				if (CurrentItem==null)
				{
					CParsingError								ParsingError=new CParsingError("TOP of STACK FOUND !",CurrentSourceCodeTerminal,CurrentItem,MSourceTerminalsQueue.GetTokenOnTop().SourceCodeIndex);
				...
				{
					CParsingError								ParsingError1=new CParsingError(string.Format("Can't find CELL in PARSING TABLE CELL for TERMINAL [{0}] !",CurrentSourceCodeTerminal),CurrentSourceCodeTerminal,CurrentItem,MSourceTerminalsQueue.GetTokenOnTop().SourceCodeIndex);

					Errors.AddError(ParsingError1);
					...
					while(FinishErrorRecovery==false)
					{
						...
						if (CurrentItem==null)
						{
							CParsingError						ParsingError2=new CParsingError("TOP of STACK FOUND !",CurrentSourceCodeTerminal,CurrentItem,MSourceTerminalsQueue.GetTokenOnTop().SourceCodeIndex);

							Errors.AddError(ParsingError2);
						...
						if (ErrorParsingTableRow.HasGotoCells()==true)
						{
							...
							while(true)
							{
								...
								MLog.AppendLine(string.Format("Current TERMINAL [{0}] IGNORED because of ERROR !",CurrentSourceCodeTerminal));
							}
						}
						else
						{
				...
				if ((ParsingTableOperation is CParsingTableOperation_Accept)==true)
				{
					...
					{
						CParsingError							ParsingError=new CParsingError("Parsing FINISHED but INPUT is NOT EMPTY !",CurrentSourceCodeTerminal,CurrentItem,MSourceTerminalsQueue.GetTokenOnTop().SourceCodeIndex);
				...
				else if ((ParsingTableOperation is CParsingTableOperation_Shift)==true)
				{
					...
					MLog.AppendLine(string.Format("Current TERMINAL [{0}] SUCCESSFULLY PROCESSED !",CurrentSourceCodeTerminal));
				}
				...
				{
					throw(new CCompilerException(string.Format("UNKNOWN PARSING TABLE OPERATION [{0}] !",ParsingTableOperation)));
				}
			}
		}
//-------------------------------------------------------------------------------------------------------
