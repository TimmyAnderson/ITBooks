...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
// DRIVER predstavuje NAJVYSSI DEVICE OBJECT v DEVICE OBJECT STACK.
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
// !!!!! POINTER na DEVICE OBJECT LOWER DRIVER.
...
NTSTATUS DispatchRoutineDeviceControl(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
	...
	{
		Status=TestIrpPassDownCopyStackLocation(DeviceObject,LowerDeviceObject,Irp);
	}
	else if (Code==IOCTL_TEST_IRP_PASS_DOWN_SKIP_STACK_LOCATION)
	{
		Status=TestIrpPassDownSkipStackLocation(DeviceObject,LowerDeviceObject,Irp);
	}
	else if (Code==IOCTL_TEST_IRP_PASS_DOWN_WITH_COMPLETION_ROUTINE)
	{
		Status=TestIrpPassDownWithCompletionRoutine(DeviceObject,LowerDeviceObject,Irp);
	}
	else if (Code==IOCTL_TEST_IRP_PASS_DOWN_ASYNCHRONOUS_PROCESSING)
	{
		Status=TestIrpPassDownAsynchronousProcessing(DeviceObject,LowerDeviceObject,Irp);
	}
	else
	{
...
void FunctionDriverUnload(PDRIVER_OBJECT DriverObject)
{
	...
	RtlInitUnicodeString(&SymbolicLink,L"\\??\\DeviceObjectStacksLayer1");

	// !!!!! Uvolni sa LOWER DEVICE OBJECT.
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath)
{
	...
	RtlInitUnicodeString(&DeviceName,L"\\Device\\DeviceObjectStacksLayer1");
	...
	// !!! Vytvori sa NAME LOWER DRIVER.
	RtlInitUnicodeString(&LowerDeviceName,L"\\Device\\DeviceObjectStacksLayer2");

	LowerDeviceObject=nullptr;

	// !!!!! CURRENT DRIVER pripoji CURRENT DEVICE OBJECT k LOWER DEVICE OBJECT cim vznikne DEVICE OBJECT STACK.
	NTSTATUS													Status3=IoAttachDevice(DeviceObject,&LowerDeviceName,&LowerDeviceObject);

	if (NT_SUCCESS(Status3)==false)
	{
		KdPrint(("!!!!! DRIVER [%wZ] FAILED to ATTACH DEVICE with ERROR CODE [%lX] !!!\n",DriverObject->DriverName,Status3));
