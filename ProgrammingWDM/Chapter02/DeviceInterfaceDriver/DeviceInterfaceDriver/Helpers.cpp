...
ETestMode CHelpers::GetTestModeValueFromFile(const WCHAR* Path)
{
	...
	for(;;)
	{
		...
		{
			DbgPrint(DRIVERLOGNAME " - FUNCTION GetTestModeValueFromFile(): Can't OPEN FILE [%s], STATUS [%X], IO STATUS [%X] !\n",FilePathANSI.Buffer,Status,IoStatusBlock.Status);
			break;
		}
		...
		{
			DbgPrint(DRIVERLOGNAME " - FUNCTION GetTestModeValueFromFile(): Can't GET FILE [%s] INFO DATA, STATUS [%X], IO STATUS [%X] !\n",FilePathANSI.Buffer,Status,IoStatusBlock.Status);
			break;
		}
		...
		if (Buffer==NULL)
		{
			DbgPrint((DRIVERLOGNAME " - FUNCTION GetTestModeValueFromFile(): Can't ALLOCATE MEMORY !\n"));
			break;
		}
		...
		{
			DbgPrint(DRIVERLOGNAME " - FUNCTION GetTestModeValueFromFile(): Can't READ to FILE [%s], STATUS [%X], IO STATUS [%X] !\n",FilePathANSI.Buffer,Status,IoStatusBlock.Status);
			break;
		}
	...
	if (HFile!=NULL)
	{
		...
		{
			DbgPrint(DRIVERLOGNAME " - FUNCTION GetTestModeValueFromFile(): Can't CLOSE HANDLE, STATUS [%X] !\n",Status);
		}
	}
