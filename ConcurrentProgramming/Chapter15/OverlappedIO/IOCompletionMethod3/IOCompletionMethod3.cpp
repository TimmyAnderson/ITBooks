...
void Test(void)
{
	...
	else
	{
		...
		if (LastError==ERROR_IO_PENDING)
		{
            ...
            while (IODone==FALSE)
            {
				// Ukazka ako je mozne urobit IO cancel.
				//CancelIoEx(hFile,&OLAP);

				// !!! Cakam na na FILE HANDLE - hFile.
				DWORD	Ret=MsgWaitForMultipleObjects(1, &hFile, FALSE, INFINITE, QS_ALLINPUT);
