...
{
//-------------------------------------------------------------------------------------------------------
	public static class CXMLScanerDeserializer
	{
//-------------------------------------------------------------------------------------------------------
		public static void DeserializeFromXML(string XML, out CInputChar[] InputChars, out CState[] States)
		{
			...
			foreach(XElement InputCharElement in InputCharsElement.Elements("InputChar"))
			{
				...
				if (TypeAttributeValue==typeof(CInputChar_AnyLetter).Name)
				{
					...
					XAttribute									AcceptUnderscoreAttribute=InputCharElement.Attribute("AcceptUnderscore");
					bool										AcceptUnderscoreValue=(bool) AcceptUnderscoreAttribute;

					InputChar=new CInputChar_AnyLetter(NameAttributeValue,AcceptUpperCaseLettersValue,AcceptLowerCaseLettersValue,AcceptUnderscoreValue);
				}
				...
				else if (TypeAttributeValue==typeof(CInputChar_OneOrMoreCharacters).Name)
				{
					...
					foreach(XElement CharacterElement in InputCharElement.Elements("Character"))
					{
						...
						else
						{
							...
							{
								throw(new CCompilerException("Value ATTRIBUTE for Character ELEMENT is EMPTY !"));
							}
					...
					{
						throw(new CCompilerException("At least ONE Character ELEMENT must be specified !"));
					}
				...
				{
					throw(new CCompilerException(string.Format("INVALID INPUT CHAR TYPE [{0}] !",TypeAttributeValue)));
				}
			...
			foreach(XElement StateElement in StatesElement.Elements("State"))
			{
				...
				else if (TypeAttributeValue==typeof(CState_AcceptingTokenEOF).Name)
				{
					XAttribute									TokenIDAttribute=StateElement.Attribute("TokenID");
					string										TokenIDAttributeValue=(string) TokenIDAttribute;

					State=new CState_AcceptingTokenEOF(NameAttributeValue,TokenIDAttributeValue);
				}
				...
				{
					throw(new CCompilerException(string.Format("INVALID STATE TYPE [{0}] !",TypeAttributeValue)));
				}
			...
			foreach(XElement StateElement in StatesElement.Elements("State"))
			{
				...
				if (State==null)
				{
					throw(new CCompilerException(string.Format("CAN'T FIND STATE [{0}] !",NameAttributeValue)));
				}
				...
				if (TypeAttributeValue==typeof(CState_NonAcceptingStart).Name || TypeAttributeValue==typeof(CState_NonAccepting).Name)
				{
					...
					foreach(XElement TransitionElement in StateElement.Elements("Transition"))
					{
						...
						if (InputChar==null)
						{
							throw(new CCompilerException(string.Format("CAN'T FIND INPUT CHAR [{0}] !",InputCharAttributeValue)));
						}
						...
						if (NewState==null)
						{
							throw(new CCompilerException(string.Format("CAN'T FIND STATE [{0}] !",NewStateAttributeValue)));
						}
