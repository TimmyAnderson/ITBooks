...
//-------------------------------------------------------------------------------------------------------
		public CSymbol GetSymbolAfterPeriod()
		{
			...
			{
				for(int Index=0;Index<MSymbols.Length;Index++)
				{
					if ((MSymbols[Index] is CPeriodLRItemSymbol)==true)
					{
		...
		public CSymbol[] GetSymbolsAfterPeriod()
		{
			...
			if (IsCompleteItem==false)
			{
				...
				for(int Index=0;Index<MSymbols.Length;Index++)
				{
					...
					{
						Symbols.Add(MSymbols[Index]);
					}
					else if ((MSymbols[Index] is CPeriodLRItemSymbol)==true)
					{
						PeriodWasFound=true;
					}
				}
