...
//------------------------------------------------------------------------------
// A global that contains application state information. This global is accessed directly by the UI fiber and indirectly by the background processing fiber.
...
VOID WINAPI LogMessage(PVOID pFlsValue)
{
	...
	// Check if we are in a fiber because this function can be called outside a fiber execution.
	if (IsThreadAFiber()==TRUE)
	{
		PVOID		pFiberData = GetCurrentFiber();
		PCTSTR		pszFlsValue = (PCTSTR)FlsGetValue(g_dwSlot);

		StringCchPrintf(szMsg, _countof(szMsg), TEXT("[0x%x - %s] %s\n"), pFiberData, (pszFlsValue == NULL) ? TEXT("'Null value'") : (PCTSTR)pszFlsValue, (pFlsValue == NULL) ? TEXT("'Null value'") : (PCTSTR)pFlsValue);
	}
	else
	{
...
void WINAPI FiberFunc(PVOID pvParam)
{
	...
	for (int x = 0; x <= nCount; x++)
	{
		...
		{
			// The UI fiber has something to do; temporarily pause counting and handle the UI events.
		...
		// Sleep for a while to exaggerate the effect; remove the call to Sleep in production code.
		Sleep(200);
	}
	...
	// Reschedule the UI thread. When the UI thread is running and has no events to process, the thread is put to sleep.
	// NOTE: If we just allow the fiber function to return, the thread and the UI fiber die -- we don't want this!
	SwitchToFiber(pFiberInfo->pFiberUI);
}
//------------------------------------------------------------------------------
...
void Dlg_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	switch (id)
	{
		...
		case IDC_COUNT:
			...
			{
				// When the user changes the count, start the background processing over from the beginning.
...
int WINAPI _tWinMain(HINSTANCE hinstExe, HINSTANCE, PTSTR pszCmdLine, int)
{
	...
	while (fQuit==FALSE)
	{
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
		{
			...
			if (IsDialogMessage(g_FiberInfo.hwnd, &msg)==FALSE)
			{
		...
		else
		{
			...
			switch (g_FiberInfo.bps)
			{
				...
				case BPS_STARTOVER:
					// User changed the count cancel the current background processing.
					if (pFiberCounter != NULL)
					{ 
						// A recalculation fiber exists; delete it so that background processing starts over from the beginning.
						DeleteFiber(pFiberCounter); 
						pFiberCounter=NULL; 
					}
				...
				case BPS_CONTINUE:
					...
					// The background processing has been paused (because a UI message showed up) or has been stopped (because the counting has completed).
					// Update the window showing which fiber is executing.
					SetDlgItemText(g_FiberInfo.hwnd, IDC_FIBER, TEXT("User interface"));

					if (g_FiberInfo.bps == BPS_DONE)
					{ 
						// The background processing ran to completion. Delete the fiber so that processing will restart next time.
						DeleteFiber(pFiberCounter); 
						pFiberCounter = NULL;
