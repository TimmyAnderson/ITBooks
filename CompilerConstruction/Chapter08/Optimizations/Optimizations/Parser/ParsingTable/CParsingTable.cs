...
using System.Text;
//-------------------------------------------------------------------------------------------------------
using CRowDictionary=System.Collections.Generic.Dictionary<Optimizations.CDFAState,Optimizations.CParsingTableRow>;
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
		public int												NumberOfShiftReduceConflicts
		{
			get
			{
				return(MNumberOfShiftReduceConflicts);
			}
		}
//-------------------------------------------------------------------------------------------------------
		public int												NumberOfReduceReduceConflicts
		{
			get
			{
				return(MNumberOfReduceReduceConflicts);
			}
		}
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
		public void InitializeParsingTable(CDFAStateCollection DFAStateCollection, CNonterminalCollection NonterminalCollection, CTerminalCollection TerminalCollection, CNonterminal AugumentedNonterminal)
		{
			...
			foreach(CDFAState DFAState in DFAStateCollection.DFAStates)
			{
				...
				if (DFAState.ContainsCompleteLR1Item()==true)
				{
					...
					if (DFAState.NFAStates.Length==1)
					{
						...
						if (Choice==null)
						{
							throw(new CCompilerException(string.Format("Can't find CHOICE for NON-TERMINAL [{0}] !",Nonterminal)));
						}
