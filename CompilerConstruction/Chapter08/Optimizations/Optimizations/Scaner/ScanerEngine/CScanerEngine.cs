...
//-------------------------------------------------------------------------------------------------------
		public CScanerEngine(CState[] States, bool LogWhiteCharsAndComments)
		{
			...
			if (StateStart==null)
			{
				throw(new CCompilerException("States DOESN'T CONTAIN START state !"));
			}

			if (States.Where(P => P.IsAcceptingState==true).Count()==0)
			{
				throw(new CCompilerException("States DOESN'T CONTAIN ACCEPTING state !"));
			}

			if (StateEOF==null)
			{
				throw(new CCompilerException("States DOESN'T CONTAIN EOF state !"));
			}
...
//-------------------------------------------------------------------------------------------------------
		private CToken GetNextToken(CCompilerErrorCollection Errors)
		{
			...
			while(MActualState.IsAcceptingState==false)
			{
				...
				if (Transition==null)
				{
					throw(new CCompilerException(string.Format("CAN'T find TRANSITION from state [{0}] for char [{1}] !",MActualState,NewChar)));
				}
