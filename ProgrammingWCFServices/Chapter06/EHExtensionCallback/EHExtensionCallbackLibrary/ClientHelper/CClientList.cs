...
{
//-------------------------------------------------------------------------------------------------------
	internal class CClientList<T> : IEHExtensionCallbackCallback where T : IEHExtensionCallbackCallback
	{
//-------------------------------------------------------------------------------------------------------
		private	object											MSyn;
...
//-------------------------------------------------------------------------------------------------------
		...
		{
			Console.WriteLine("Client OnEvent() STARTED !");
			...
			lock(MSyn)
			{
				for(int i=0;i<MList.Count;i++)
				{
					try
					{
						...
						MList[i].Client.OnEvent(Message);
					}
					catch(FaultException<InvalidOperationException> E)
					{
						// Proxy prezije.
						Console.WriteLine(string.Format("FaultCallback() - FaultException<InvalidOperationException>: Msg: {0}, Detail: {1} !",E.Message,E.Detail));
					}
