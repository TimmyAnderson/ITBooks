...
DEFINE_GUID(GUID_MY_CUSTOM_PNP_EVENT, 0x28ce01a2, 0x9436, 0x11d2, 0x81, 0xb5, 0x0, 0xc0, 0x4f, 0xa3, 0x30, 0xa6);
//-------------------------------------------------------------------------------------------------------
// !!! CUSTOM NOTIFICATION STRUCTURE.
struct MY_CUSTOM_NOTIFICATION_STRUCTURE : public TARGET_DEVICE_CUSTOM_NOTIFICATION
{
	WCHAR														MCustomData[100];
};
//-------------------------------------------------------------------------------------------------------
...
#define IOCTL_SYNCHRONOUS_CUSTOM_NOTIFICATION CTL_CODE(FILE_DEVICE_UNKNOWN,0x802,METHOD_BUFFERED,FILE_ANY_ACCESS)
#define IOCTL_ASYNCHRONOUS_CUSTOM_NOTIFICATION CTL_CODE(FILE_DEVICE_UNKNOWN,0x803,METHOD_BUFFERED,FILE_ANY_ACCESS)
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
...
void IRPCompleteSimulator(PVOID Parameter)
{
	...
	/**/
	// !!!!! CODE vykona BUG CHECK pri vykonani FUNCTION IoReleaseRemoveLock(). Pravdepodobna pricina spociva v tom, ze WINDOWS detekuje to, ze DRIVER NEUVOLNIL WORKER THREAD v ktorom CODE bezi. V popise k BUG CHECK 0xCE sa pise: [This driver failed to cancel lookaside lists, DPCs, worker threads, or other such items before unload.].

	// !!! POCKAM 30 SEKUND.
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): will WAIT for 30 SECONDS !\n");
	...
	// !!!!! TENTO CODE uz NIE JE CHRANENY REMOVE LOCK a teda sa moze stat, ze ak by WINDOWS uvolnil DRIVER SKOR ako by sa zavolal nasledujuci CODE doslo by k BUG CHECK. Nanestastie WDM NEPOSKYTUJE MOZNOST, ako tejto situacii ZABRANIT.
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): AFTER RELEASE LOCK !\n");

	/*
	// !!!!! TENTO CODE uz NIE JE CHRANENY REMOVE LOCK a teda sa moze stat, ze ak by WINDOWS uvolnil DRIVER SKOR ako by sa zavolal nasledujuci CODE doslo by k BUG CHECK. Nanestastie WDM NEPOSKYTUJE MOZNOST, ako tejto situacii ZABRANIT.

	// !!!!! Pokial sa pocas 30 SEKUND uvolni DRIVER z MEMORY dojde k BUG CHECK typu [DRIVER_UNLOADED_WITHOUT_CANCELLING_PENDING_OPERATIONS].
	// !!!!! Je to dosledok toho, ze sa UVOLNIL REMOVE LOCK co umoznilo odstranenie DRIVER z MEMORY. Ked vsak uplynul TIMEOUT, WINDOWS chcel pokracovat vo vykonavani tejto FUNCTION, ktora uz bola ODSTRANENA z MEMORY.
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): will WAIT for 30 SECONDS !\n");
...
//-------------------------------------------------------------------------------------------------------
NTSTATUS ProcessIOControlSynchronousCustomNotification(PIRP, DEVICE_EXTENSION* DeviceExtension, ULONG, ULONG, ULONG, ULONG&)
{
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): PROCESSING IO CONTROL !\n");

	NTSTATUS													Status=STATUS_SUCCESS;

	MY_CUSTOM_NOTIFICATION_STRUCTURE							Notify;

	Notify.Version=1;
	Notify.Size=sizeof(Notify);
	Notify.Event=GUID_MY_CUSTOM_PNP_EVENT;
	Notify.FileObject=NULL;
	Notify.NameBufferOffset=FIELD_OFFSET(MY_CUSTOM_NOTIFICATION_STRUCTURE,MCustomData)-FIELD_OFFSET(MY_CUSTOM_NOTIFICATION_STRUCTURE,CustomDataBuffer);

	wcscpy_s(Notify.MCustomData,L"Hello, world !");

	// !!!!! POZOR. TENTO PNP EVENT NOTIFICATION DRIVER [PNPNotificationDriver] NEZACHYTI, pretoze ten zachytava iba PNP EVENTS typu [EventCategoryDeviceInterfaceChange] a NIE PNP EVENTS typu [EventCategoryTargetDeviceChange].
	// !!!!! AVSAK APPLICATION [PNPNotificationsByHandle] PNP EVENT ZACHYTI.
	Status=IoReportTargetDeviceChange(DeviceExtension->PDO,&Notify);

	if (NT_SUCCESS(Status)==true)
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): FUNCTION IoReportTargetDeviceChange() SUCCEEDED !\n");

		Status=STATUS_SUCCESS;
	}
	else
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): FUNCTION IoReportTargetDeviceChange() FAILED [%X] !\n",Status);
	}

	return(Status);
}
//-------------------------------------------------------------------------------------------------------
NTSTATUS ProcessIOControlAsynchronousCustomNotification(PIRP, DEVICE_EXTENSION* DeviceExtension, ULONG, ULONG, ULONG, ULONG&)
{
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): PROCESSING IO CONTROL !\n");
	...
	// !!!!! POZOR. TENTO PNP EVENT NOTIFICATION DRIVER [PNPNotificationDriver] NEZACHYTI, pretoze ten zachytava iba PNP EVENTS typu [EventCategoryDeviceInterfaceChange] a NIE PNP EVENTS typu [EventCategoryTargetDeviceChange].
	// !!!!! AVSAK APPLICATION [PNPNotificationsByHandle] PNP EVENT ZACHYTI.
	Status=IoReportTargetDeviceChangeAsynchronous(DeviceExtension->PDO,&Notify,NULL,NULL);

	if (NT_SUCCESS(Status)==true)
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): FUNCTION IoReportTargetDeviceChangeAsynchronous() SUCCEEDED !\n");

		Status=STATUS_SUCCESS;
	}
	else
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): FUNCTION IoReportTargetDeviceChangeAsynchronous() FAILED [%X] !\n",Status);
	}
...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	for(;;)
	{
		...
		Status=IoRegisterDeviceInterface(PhysicalDeviceObject,&GUID_DEVINTERFACE_PLUG_AND_PLAY_DRIVER,NULL,&DeviceExtensionData->InterfaceName);
...
NTSTATUS DispatchRead(PDEVICE_OBJECT DeviceObject, PIRP InterruptRequestPacket)
{
	...
	// !!!!! CODE NESPOSOBI BUG CHECK, pretoze WINDOWS ziska EXTRA REFERENCIU na DEVICE OBJECT pred zaslanim IRP do DRIVER a ta sa neuvolni az kym DRIVER neukonci spracovanie IRP. Preto k PREDCASNEMU ODSTRANENIU DRIVER z MEMORY NEDOCHADZA.
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): will WAIT for 30 SECONDS !\n");
...
NTSTATUS DispatchSystemControl(PDEVICE_OBJECT DeviceObject, PIRP InterruptRequestPacket)
{
	...
	switch(Code)
	{
		...
		case IOCTL_SYNCHRONOUS_CUSTOM_NOTIFICATION:
		{
			Status=ProcessIOControlSynchronousCustomNotification(InterruptRequestPacket,DeviceExtensionData,Code,InputBufferLength,OutputBufferLength,Info);
		}
		break;

		case IOCTL_ASYNCHRONOUS_CUSTOM_NOTIFICATION:
		{
			Status=ProcessIOControlAsynchronousCustomNotification(InterruptRequestPacket,DeviceExtensionData,Code,InputBufferLength,OutputBufferLength,Info);
		}
		break;
