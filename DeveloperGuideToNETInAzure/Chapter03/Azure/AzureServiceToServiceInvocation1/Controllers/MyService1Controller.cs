...
//----------------------------------------------------------------------------------------------------------------------
		[HttpGet("IsAlive")]
		public string CallIsAlive()
		{
			return("SERVICE 1 is ALIVE.");
		}
//----------------------------------------------------------------------------------------------------------------------
		[HttpGet]
		public async Task<string> CallService()
		{
			...
			if (Program.RUN_CONFIGURATION==ERunConfiguration.E_LOCAL_NO_DAPR)
			{
				ServiceUrl=$"{BASE_URL_SERVICE_2_LOCAL}MyService2";
			}
			else if (Program.RUN_CONFIGURATION==ERunConfiguration.E_LOCAL_WITH_DAPR)
			{
				// !!!!! Pri komunikacii cez DAPR HTTP CLIENT sa pouziva iba LOCAL ADDRESS.
				ServiceUrl="MyService2";
			}
			else if (Program.RUN_CONFIGURATION==ERunConfiguration.E_AZURE_WITH_DAPR)
			{
				// !!!!! Pri komunikacii cez DAPR HTTP CLIENT sa pouziva iba LOCAL ADDRESS.
				ServiceUrl="MyService2";
			}
			...
			{
				throw(new InvalidOperationException($"UNSUPPORTED VALUE [{Program.RUN_CONFIGURATION}]."));
			}
