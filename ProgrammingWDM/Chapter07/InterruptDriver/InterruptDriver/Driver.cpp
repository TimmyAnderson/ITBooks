...
#define IOCTL_SYNCHRONIZATION_EXECUTION CTL_CODE(FILE_DEVICE_UNKNOWN,0x802,METHOD_BUFFERED,FILE_ANY_ACCESS)
#define IOCTL_INTERRUPT_SPIN_LOCK CTL_CODE(FILE_DEVICE_UNKNOWN,0x803,METHOD_BUFFERED,FILE_ANY_ACCESS)
#define IOCTL_CUSTOM_DPC CTL_CODE(FILE_DEVICE_UNKNOWN,0x804,METHOD_BUFFERED,FILE_ANY_ACCESS)
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
...
BOOLEAN MySynchronizationFunction(PVOID)
{
	// !!!!! Tento CODE je uz vykonavany so ziskanym SPIN LOCK a v [IRQL=DIRQL].

	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MY SYNCHRONIZATION FUNCTION CALLED 1 !\n");

	// Umyselne spomalenie CODE, aby sa demonstrovalo, ze pocas jeho behu NEDOCHADZA k INTERRUPTS.
	// !!!!! CODE by mal byt natolko zlozity, aby cas jeho vykonavania bol aspon 200 MS.
	for(int Index=0;Index<100000;Index++)
	{
		int														DummyArgument=6843726;

		// Nejaky casovo narocnejsi CODE.
		for(int LocalIndex=0;LocalIndex<10000;LocalIndex++)
		{
			if ((LocalIndex % 2)==0)
			{
				DummyArgument/=34;
		...
		if (((Index+1) % 10000)==0)
		{
			DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MY SYNCHRONIZATION FUNCTION LOOP [%d], IRQL [%d] !\n",(Index+1),KeGetCurrentIrql());
		}
	}

	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MY SYNCHRONIZATION FUNCTION CALLED 2 !\n");

	return(TRUE);
}
//-------------------------------------------------------------------------------------------------------
NTSTATUS ProcessIOControlSynchronizeEvent(PIRP, DEVICE_EXTENSION* DeviceExtensionData, ULONG, ULONG, ULONG, ULONG&)
{
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): PROCESSING IO CONTROL !\n");

	// !!!!! FUNCTION KeSynchronizeExecution() SYNCHRONNE zavola CUSTOM CALLBACK FUNCTION MySynchronizationFunction(), pricom jej CODE bezi so SPIN LOCK a IRQL, ktore su asociovane s danym INTERRUPT.
	BOOLEAN														MySynchronizationFunctionReturnValue=KeSynchronizeExecution(DeviceExtensionData->InterruptObject,MySynchronizationFunction,DeviceExtensionData);

	if (MySynchronizationFunctionReturnValue==TRUE)
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MY SYNCHRONIZATION FUNCTION FINISHED with RETURN VALUE [TRUE] !\n");
	}
	else
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MY SYNCHRONIZATION FUNCTION FINISHED with RETURN VALUE [FALSE] !\n");
	}
...
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
NTSTATUS ProcessIOControlInterruptSpinLock(PIRP, DEVICE_EXTENSION* DeviceExtensionData, ULONG, ULONG, ULONG, ULONG&)
{
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): PROCESSING IO CONTROL !\n");

	// !!!!! FUNCTION KeAcquireInterruptSpinLock() ziska SPIN LOCK asociovany s danym INTERRUPT a nastavi IRQL na hodnotu priradenu danemu INTERRUPT.
	KIRQL														OldIRQL=KeAcquireInterruptSpinLock(DeviceExtensionData->InterruptObject);

	// !!!!! Tento CODE je uz vykonavany so ziskanym SPIN LOCK a v [IRQL=DIRQL].

	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): SYNCHRONIZED CODE EXECUTING 1 !\n");
	...
	for(int Index=0;Index<100000;Index++)
	{
		...
		if (((Index+1) % 10000)==0)
		{
			DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): SYNCHRONIZED CODE LOOP [%d], IRQL [%d] !\n",(Index+1),KeGetCurrentIrql());
		}
	}

	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): SYNCHRONIZED CODE EXECUTING 2 !\n");

	// !!! Uvolni SPIN LOCK asoicovany s danym INTERRUPT a znizi IRQL na stanovanu hodnotu.
	KeReleaseInterruptSpinLock(DeviceExtensionData->InterruptObject,OldIRQL);
...
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
NTSTATUS ProcessIOControlCustomDPC(PIRP, DEVICE_EXTENSION* DeviceExtensionData, ULONG, ULONG, ULONG, ULONG&)
{
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): PROCESSING IO CONTROL !\n");

	// !!! Vykona sa SCHEDULING CUSTOM DPC.
	BOOLEAN														ItemInserted=KeInsertQueueDpc(&DeviceExtensionData->CustomDPC,(PVOID) 1111,(PVOID) 2222);

	if (ItemInserted!=FALSE)
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): ITEM ADDED to CUSTOM DPC !\n");
	}
	else
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): ITEM FAILED to ADD to CUSTOM DPC !\n");
	}
...
BOOLEAN MyInterruptServiceRoutine(PKINTERRUPT, PVOID Context)
{
	...
	if ((Value % 20)==0)
	{
		...
		// !!! Vykona sa SCHEDULING CUSTOM DPC.
		BOOLEAN													ItemInserted=KeInsertQueueDpc(&DeviceExtensionData->CustomDPC,(PVOID) 100,(PVOID) 200);

		if (ItemInserted!=FALSE)
		{
			DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MyInterruptServiceRoutine() ADDED ITEM to CUSTOM DPC !\n");
		}
		else
		{
			DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): MyInterruptServiceRoutine() FAILED to ADD ITEM to CUSTOM DPC !\n");
		}
	}
...
_IRQL_requires_same_
VOID CustomDPC(PKDPC, PVOID, PVOID SystemArgument1, PVOID SystemArgument2)
{
	DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): CustomDPC() CALLED ! SystemArgument1 [%p], SystemArgument2 [%p] !\n",SystemArgument1,SystemArgument2);
}
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	for(;;)
	{
		...
		// !!! Zaregistrovanie CUSTOM DPC.
		KeInitializeDpc(&DeviceExtensionData->CustomDPC,CustomDPC,NULL);
		...
		Status=IoRegisterDeviceInterface(PhysicalDeviceObject,&GUID_DEVINTERFACE_INTERRUPT_DRIVER,NULL,&DeviceExtensionData->InterfaceName);
...
NTSTATUS DispatchSystemControl(PDEVICE_OBJECT DeviceObject, PIRP InterruptRequestPacket)
{
	...
	switch(Code)
	{
		...
		case IOCTL_SYNCHRONIZATION_EXECUTION:
		{
			Status=ProcessIOControlSynchronizeEvent(InterruptRequestPacket,DeviceExtensionData,Code,InputBufferLength,OutputBufferLength,Info);
		}
		break;

		case IOCTL_INTERRUPT_SPIN_LOCK:
		{
			Status=ProcessIOControlInterruptSpinLock(InterruptRequestPacket,DeviceExtensionData,Code,InputBufferLength,OutputBufferLength,Info);
		}
		break;

		case IOCTL_CUSTOM_DPC:
		{
			Status=ProcessIOControlCustomDPC(InterruptRequestPacket,DeviceExtensionData,Code,InputBufferLength,OutputBufferLength,Info);
		}
		break;
