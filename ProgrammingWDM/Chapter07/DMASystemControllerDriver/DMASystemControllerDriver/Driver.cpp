...
void ProcessWriteIRP(PDEVICE_EXTENSION DeviceExtension, PIRP InterruptRequestPacket)
{
	...
	PSTR														DataToWrite=(PSTR) MmGetSystemAddressForMdlSafe(CurrentMDL,NormalPagePriority);
...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	for(;;)
	{
		...
		Status=IoRegisterDeviceInterface(PhysicalDeviceObject,&GUID_DEVINTERFACE_DMA_SYSTEM_CONTROLLER_DRIVER,NULL,&DeviceExtensionData->InterfaceName);
...
NTSTATUS __stdcall StartDevice(PDEVICE_OBJECT DeviceObject, PCM_PARTIAL_RESOURCE_LIST RawList, PCM_PARTIAL_RESOURCE_LIST TranslatedList)
{
	...
	{
		DbgPrint(DRIVERLOGNAME " - FUNCTION " __FUNCTION__ "(): Didn't get expected INTERRUPT HW RESOURCE !\n");
		return(STATUS_DEVICE_CONFIGURATION_ERROR);
	}
