...
NTSTATUS IrpPassDownWithCompletionRoutineCompletionRoutine(PDEVICE_OBJECT DeviceObject, PIRP Irp, PVOID Context)
{
	...
	KdPrint(("!!!!!!!!!! DRIVER [%wZ] ROUTINE [%s] UPPER DRIVER - COMPLETION ROUTINE CALLED !!!\n",DeviceObject->DriverObject->DriverName,FunctionName));
...
NTSTATUS TestIrpPassDownWithCompletionRoutine(DEVICE_OBJECT* DeviceObject, DEVICE_OBJECT* LowerDeviceObject, IRP* Irp)
{
	...
	IoSetCompletionRoutine(Irp,IrpPassDownWithCompletionRoutineCompletionRoutine,nullptr,true,true,true);
