...
void StandardTest(void)
{
	...
	try
	{
		...
		DefaultAppDomain->CreateNewDomainAndNewThread(bstr_t(L"My DOMAIN 1"));
		DefaultAppDomain->CreateNewDomainAndNewThread(bstr_t(L"My DOMAIN 2"));
...
//-------------------------------------------------------------------------------------------------------
// !!!!! LIBRARY MyDomainManager.dll MUSI byt NAKOPIROVANA do TOHO ISTEHO DIRECTORY ako .EXE FILE.
// ????? Priklad mi NEFNGOVAL a CODE obsahujuci spustenie THREADU sa vykonal AJ NAPRIEK definovanej HOST PROTECTION. Je mozne, ze problem je v tom, ze APP DOMAIN NEBEZALA ako PARTIALY TRUSTED, kedy sa HOST PROTECTION NEAPLIKUJE.
void EnforcingHostProtectionTest(void)
{
	HRESULT														HR;
	CSmartCOMPointer<ICLRMetaHost>								MetaHost=NULL;
	CSmartCOMPointer<ICLRRuntimeInfo>							RuntimeInfo=NULL;
	...
	try
	{
		...
		// !!!!! Zapnem HOST PROCETION obmedzeniem CATEGORIES, ktore MANAGED CODE moze pouzivat.
		// !!!!! CLASSES a METHODS z eExternalThreading CATEGORY sa NEBUDU MOCT POUZIVAT.
		if (FAILED(HR=CLRHostProtectionManager->SetProtectedCategories(EApiCategories::eExternalThreading)))
		{
			throw(CHelpers::PrepareLogMessage(L"SetProtectedCategories() FAILED.",HR));
		}
