...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ExtendedTimersDeleteWaitTimerCallback(PEX_TIMER Timer, PVOID Context)
{
...
//----------------------------------------------------------------------------------------------------------------------
void ExtendedTimersDeleteWaitTimerDeleteCallback(PVOID Context)
{
	// !!! CODE sa vykonava v IRQL [DISPATCH_LEVEL].
...
NTSTATUS TestExtendedTimersDeleteWait(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
...

	{
		...
		PEX_TIMER												Timer=ExAllocateTimer(ExtendedTimersDeleteWaitTimerCallback,nullptr,EX_TIMER_HIGH_RESOLUTION);
		...
		if (Timer!=nullptr)
		{
			...
			DeleteParameters.DeleteCallback=ExtendedTimersDeleteWaitTimerDeleteCallback;
			DeleteParameters.DeleteContext=nullptr;

			// !!! Uvolni sa HIGH RESOLUTION TIMER.
			// !!!!! Zaroven sa CAKA na SKONCENIE TIMER, ale NEROBI sa CANCEL existujuceho REQUEST na TIMER.
			BOOLEAN												TimerCancelledInDelete=ExDeleteTimer(Timer,FALSE,TRUE,&DeleteParameters);
