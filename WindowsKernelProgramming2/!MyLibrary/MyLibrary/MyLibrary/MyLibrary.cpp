...
NTSTATUS DispatchRoutineDeviceControl(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
	...
	if (Code==IOCTL_TEST_MUTEX)
	{
		Status=TestMutex(DeviceObject,Irp);
	}
	else if (Code==IOCTL_TEST_FAST_MUTEX)
	{
		Status=TestFastMutex(DeviceObject,Irp);
	}
	else if (Code==IOCTL_TEST_MEMORY_OPERATORS)
	{
