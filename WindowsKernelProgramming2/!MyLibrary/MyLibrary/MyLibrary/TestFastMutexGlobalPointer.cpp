...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
ULONG64															FastMutexGlobalPointerSharedVariable=0;
CFastMutex*														FastMutexGlobalPointer=nullptr;
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexGlobalPointer1(PVOID StartContext)
{
	const char*													FunctionName=__FUNCTION__;

	CFastMutex*													Mutex=static_cast<CFastMutex*>(StartContext);
	...
	for(LONG64 Index=0;Index<LONG64(1000*1000);Index++)
	{
		...
		{
			FastMutexGlobalPointerSharedVariable++;
		}
	}
...
//----------------------------------------------------------------------------------------------------------------------
void ThreadFastMutexGlobalPointer2(PVOID StartContext)
{
	const char*													FunctionName=__FUNCTION__;
...
NTSTATUS TestFastMutexGlobalPointer(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
	...
	{
		FastMutexGlobalPointer=new (EPoolFlags::E_POOL_FLAG_PAGED) CFastMutex();

		if (FastMutexGlobalPointer!=nullptr)
		{
			{
				...
				do
				{
					Status=CHelpers::CreateThread(DeviceObject,"111",ThreadFastMutexGlobalPointer1,FastMutexGlobalPointer,&ThreadObject1);
					...
					Status=CHelpers::CreateThread(DeviceObject,"222",ThreadFastMutexGlobalPointer2,FastMutexGlobalPointer,&ThreadObject2);
					...
					KdPrint(("!!!!!!!!!! ROUTINE [%s] - SHARED VARIABLE [%I64d].\n",FunctionName,FastMutexGlobalPointerSharedVariable));
				}
				while(false);
			...
			delete(FastMutexGlobalPointer);
			FastMutexGlobalPointer=nullptr;
		}
		else
		{
