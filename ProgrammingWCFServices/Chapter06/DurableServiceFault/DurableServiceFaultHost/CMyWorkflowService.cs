...
//-------------------------------------------------------------------------------------------------------
	...
	[DurableService(UnknownExceptionAction=UnknownExceptionAction.AbortInstance)]
	//[DurableService(UnknownExceptionAction=UnknownExceptionAction.TerminateInstance)]
...
//-------------------------------------------------------------------------------------------------------
		...
		{
			PrintState("SetFirstName - BEFORE SET");

			MFirstName=FirstName;

			PrintState("SetFirstName - AFTER SET");
		}
//-------------------------------------------------------------------------------------------------------
		[DurableOperation(CanCreateInstance=false,CompletesInstance=false)]
		public void SetLastName(string LastName)
		{
			PrintState("SetLastName - BEFORE SET");

			MLastName=LastName;

			PrintState("SetLastName - AFTER SET");
		}
//-------------------------------------------------------------------------------------------------------
		[DurableOperation(CanCreateInstance=true,CompletesInstance=false)]
		public void SetLastNameButThrowException(string LastName)
		{
			PrintState("ThrowException - BEFORE SET");

			MLastName=LastName;

			PrintState("ThrowException - AFTER SET");
