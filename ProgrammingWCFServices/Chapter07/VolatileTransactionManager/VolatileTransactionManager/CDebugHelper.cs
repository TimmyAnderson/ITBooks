...
//-------------------------------------------------------------------------------------------------------
		public static string GetMethodCallString(int Position)
		{
			...
			return(string.Format("{0}.{1}() called {2} !",SF.GetType().Name,SF.GetMethod().Name,Position));
		}
//-------------------------------------------------------------------------------------------------------
		public static string GetTransactionString(Transaction T)
		{
			...
			if (T==null)
				return(string.Format("{0}.{1}() TRANSACTION (NONE)",SF.GetType().Name,SF.GetMethod().Name));
			else
				return(string.Format("{0}.{1}() TRANSACTION ({2},{3})",SF.GetType().Name,SF.GetMethod().Name,(T.TransactionInformation.LocalIdentifier ?? "NONE"),((T.TransactionInformation.DistributedIdentifier!=Guid.Empty) ? T.TransactionInformation.DistributedIdentifier.ToString() : "NONE")));
		}
//-------------------------------------------------------------------------------------------------------
	}
//-------------------------------------------------------------------------------------------------------
