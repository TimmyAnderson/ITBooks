...
//-------------------------------------------------------------------------------------------------------
		...
		static void Test1()
		{
			...
			try
			{
				Client=new CPerCallMSMQProxy(TCPBinding,QueueName);
...
//-------------------------------------------------------------------------------------------------------
		...
		static void Test6()
		{
			...
			Random								RND=new Random();
			string								QueueName;
			string								Message=string.Format("TEST6 - {0}",RND.Next(100,999));
			...
			try
			{
				...
				using(TransactionScope T=new TransactionScope())
				{
					// Service nebude pouzivat tranakcie ale na tom nezalezi.
					Client.PrintText(Message);

					// !!! Neurobim commit mam ROLLBACK a tym padom sa volanie nezaradi do Queue.
					// !!!!! SERVICE OPERACIA sa VOBEC NEZAVOLA.
				}
