...
//-------------------------------------------------------------------------------------------------------
		private ESymbolType ConvertArrayTypeToBaseType(ESymbolType ArrayType)
		{
			...
			{
				throw(new CCompilerException(string.Format("UNEXPECTED VALUE of ARRAY TYPE [{0}] !",ArrayType.EXT_GetFriendlyName())));
			}
		}
//-------------------------------------------------------------------------------------------------------
		private ESymbolType GetSymbolType(CTerminal_Keyword TypeKeyword, bool IsArray)
		{
			...
			{
				throw(new CCompilerException(string.Format("UNEXPECTED VALUE of TYPE VALUE [{0}] !",TypeKeyword.KeywordLexema)));
			}
		}
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_VARIABLE_DECLARATION_Choice2(CParseTreeNode Node, CParseTreeTraverseItem TraverseItem)
		{
			...
			if (Type!=null)
			{
				if (MCurrentTable.ContainsSymbolInCurrentOrParentScopes(SymbolName)==true)
				{
					...
					else
					{
						...
						{
							throw(new CCompilerException(string.Format("INVALID TYPE of CONST !",Node)));
						}
					}
				}
				else
			...
			{
				throw(new CCompilerException(string.Format("NODE doesn't contain CONST TYPE INFORMATION !",Node)));
			}
		}
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_STATEMENT_Choice5(CParseTreeNode Node, CParseTreeTraverseItem TraverseItem)
		{
			...
			{
				throw(new CCompilerException(string.Format("NODE doesn't contain RETURN TYPE INFORMATION !",Node)));
			}
...
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_EXPRESSION_STATEMENT_Choice1(CParseTreeNode Node, CParseTreeTraverseItem TraverseItem)
		{
			...
			{
				throw(new CCompilerException(string.Format("NODE doesn't contain TYPE INFORMATION !",TreeNodeID.Token.Lexema)));
			}
		}
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_SELECTION_STATEMENT_Choice1(CParseTreeNode Node, CParseTreeTraverseItem TraverseItem)
		{
			...
			{
				throw(new CCompilerException(string.Format("NODE doesn't contain TYPE INFORMATION !",Node)));
			}
...
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_FACTOR_Choice5(CParseTreeNode Node, CParseTreeTraverseItem TraverseItem)
		{
			...
			{
				CSemanticError									SemanticError=new CSemanticError(string.Format("LITERAL contains INVALID ESCAPE SEQUENCE !"),STRING_LITERAL.Token);
					...
					if ((SymbolFromSymbolTable.SymbolTableData is CSymbolTableData_Function)==true)
					{
						...
						if (FunctionTable!=null)
						{
							...
							if (FunctionParamTypes.Length==Params.Length)
							{
								...
								for (int Index=0;Index<FunctionParamTypes.Length;Index++)
								{
									ESymbolType					FunctionType=FunctionParamTypes[Index].SymbolType;
									ESymbolType					ParamType=Params[Index].SymbolType;

									if (FunctionType!=ParamType)
									{
										CSemanticError			SemanticError=new CSemanticError(string.Format("{0}. PARAMETER in FUNCTION [{1}] has TYPE [{2}] but EXCPECTED TYPE is [{3}] !",(Index+1),SymbolName,ParamType,FunctionType),TreeNodeID.Token);
						...
						{
							throw(new CCompilerException(string.Format("Can't FIND FUNCTION TABLE for FUNCTION [{0}] !",SymbolName)));
						}
					}
					else
					{
...
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_ARG_LIST_Choice1(CParseTreeNode Node, CParseTreeTraverseItem TraverseItem)
		{
			...
			{
				throw(new CCompilerException(string.Format("NODE doesn't contain TYPE or PARAM TYPES INFORMATION !",Node)));
			}
		}
//-------------------------------------------------------------------------------------------------------
