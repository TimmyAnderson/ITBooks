...
void TestInitialization(void)
{
	...
	vector<wstring>												Vector3{L"3",L"4",L"6",L"8",L"2"};

	PrintCollection(L"VECTOR 3",Vector3);

	// !!! TEMPLATE FUNCTION make_move_iterator<TIterator>() vytvara instanciu CLASS [std::move_iterator<TIterator>], ktora reprezentuje MOVE ITERATOR. Pri iterovani ITEMS cez MOVE ITERATOR su ITEMS MOVED do DESTINATION COLLECTION.
	vector<wstring>												Vector4(make_move_iterator(Vector3.begin()),make_move_iterator(Vector3.end()));

	PrintCollection(L"VECTOR 4",Vector4);

	// Kedze sa pouzil MOVE OPERATOR, COLLECTION obsahuje iba EMPTY STRINGS.
	PrintCollection(L"VECTOR 3",Vector3);

	PrintLineSeparator();

	wstring														Array[]{L"3",L"4",L"6",L"8",L"2"};

	// !!! Pomocou TEMPLATE FUNCTION begin() a TEMPLATE FUNCTION cend() je mozne kopirovat C-ARRAYS do CONTAINERS.
	vector<wstring>												Vector5(begin(Array),end(Array));

	PrintCollection(L"VECTOR 5",Vector5);
	...
	// CLASS [istream_iterator] reprezentuje ITERATOR, ktorym je mozne iterovat ITEMS v STREAM. Pomocou tohto ITERATOR je mozne presunut ITEMS zo STREAMS do CONTAINERS.
	vector<int>													Vector6{istream_iterator<int>(Stream),istream_iterator<int>()};

	PrintCollection(L"VECTOR 6",Vector6);
...
void TestSwap(void)
{
	...
	// !!! Po MOVE OPERATION je obsah SOURCE CONTAINER NEDEFINOVANY.
	PrintCollection(L"VECTOR 1",Vector1);

	PrintLineSeparator();

	vector<int>													Vector3{3,4,6,8,2};
	vector<int>													Vector4{13,14,16,18,12};
	...
	// !!! METHOD swap() vymiena POINTERS medzi 2 CONTAINERS.
...
void TestComparisonOperators(void)
{
	...
	unordered_set<int>												UnorderedSet1{3,4,6,8,2};
	unordered_set<int>												UnorderedSet2{3,4,16,8,2};
	...
	// UNORDERED CONTAINERS maju iba OPERATOR== a OPERATOR!=.
	wcout << L"OPERATOR== [" << ((UnorderedSet1==UnorderedSet2) ? L"TRUE" : L"FALSE") << L"] !" << endl;
	wcout << L"OPERATOR!= [" << ((UnorderedSet1!=UnorderedSet2) ? L"TRUE" : L"FALSE") << L"] !" << endl;
...
void TestArrayAsCArray(void)
{
	...
	wcout << L"TEXT [" << Array.data() << L"] !" << endl;
...
void TestVectorAsCArray(void)
{
	...
	wcout << L"TEXT [" << Vector.data() << L"] !" << endl;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void TestStringCollection(void)
{
	PrintLineSeparator();
	...
	// !!! CLASS [wstring] je TIEZ STL COLLECTION.
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void TestReferenceSemanticsSharedPointer(void)
{
	PrintLineSeparator();

	vector<shared_ptr<CComplexClass>>							Vector1;

	Vector1.push_back(shared_ptr<CComplexClass>(new CComplexClass(10)));
	Vector1.push_back(shared_ptr<CComplexClass>(new CComplexClass(20)));
	Vector1.push_back(shared_ptr<CComplexClass>(new CComplexClass(30)));

	PrintCollectionSharedPointer(L"VECTOR 1",Vector1);

	PrintLineSeparator();

	{
		// !!! Kedze sa pouzivaju SHARED POINTERS, tak sa NEVOLAJU COPY CONSTRUCTORS ani COPY OPERATORS.
		vector<shared_ptr<CComplexClass>>						Vector2(Vector1);

		PrintCollectionSharedPointer(L"VECTOR 2",Vector2);

		PrintLineSeparator();

		for(shared_ptr<CComplexClass>& Item : Vector2)
		{
	...
	// !!! Kedze sa pouzili SHARED POINTERS, zmena sa prejavi AJ v COLLECTION 'Vector1'.
	PrintCollectionSharedPointer(L"VECTOR 1",Vector1);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void TestReferenceSemanticsReferenceWrapper(void)
{
	PrintLineSeparator();
	
	vector<CComplexClass>										Vector1;
	...
	{
		vector<reference_wrapper<CComplexClass>>				Vector2;

		for(CComplexClass& Item : Vector1)
		{
		...
		for(reference_wrapper<CComplexClass>& Item : Vector2)
		{
			Item.get().SetValue(Item.get().GetValue()*2);
		}
	...
	// !!! Kedze sa pouzili REFERENCE WRAPPERS, zmena sa prejavi AJ v COLLECTION 'Vector1'.
	PrintCollection(L"VECTOR 1",Vector1);
...
int main(void)
{
	...
	//TestSTLCollection();
	//TestReferenceSemanticsSharedPointer();
	//TestReferenceSemanticsReferenceWrapper();
