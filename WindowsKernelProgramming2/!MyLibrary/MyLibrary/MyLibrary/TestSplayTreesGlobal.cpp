...
#include "Helpers\CSplayTreeGlobal.h"
#include "SMyTreeKey.h"
#include "SMyTreeItem.h"
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
CSplayTreeGlobal<SMyTreeKey,SMyTreeItem>						SplayTreesGlobalTree;
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
PVOID SplayTreesGlobalInsertItem(CSplayTreeGlobal<SMyTreeKey,SMyTreeItem>& Tree, SMyTreeItem& TreeItem)
{
...
//----------------------------------------------------------------------------------------------------------------------
void SplayTreesGlobalDeleteItem(CSplayTreeGlobal<SMyTreeKey,SMyTreeItem>& Tree, SMyTreeKey& TreeKey)
{
...
//----------------------------------------------------------------------------------------------------------------------
void SplayTreesGlobalGetItem(CSplayTreeGlobal<SMyTreeKey,SMyTreeItem>& Tree, SMyTreeKey& TreeKey)
{
...
NTSTATUS TestSplayTreesGlobal(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
...

	{
		...
		SplayTreesGlobalTree.Init(EPoolFlags::E_POOL_FLAG_PAGED);
		...
		do
		{
			...
			{
				if (SplayTreesGlobalInsertItem(SplayTreesGlobalTree,TreeItems[Index])==nullptr)
				{
					InsertsFailed=true;
					break;
				}
			...
			while(true)
			{
				PVOID											RawTreeItem=SplayTreesGlobalTree.EnumerateWithoutSplaying(RestartKey);
			...
			SplayTreesGlobalInsertItem(SplayTreesGlobalTree,TreeItems[0]);

			// !!! DELETE existujuceho ITEM.
			SplayTreesGlobalDeleteItem(SplayTreesGlobalTree,TreeItems[3]);

			// !!! Pokus o DELETE EXISTUJUCEHO ITEM. DELETE zlyha.
			SplayTreesGlobalDeleteItem(SplayTreesGlobalTree,TreeItems[3]);

			// !!! Ziskanie ITEM podla KEY.
			SplayTreesGlobalGetItem(SplayTreesGlobalTree,TreeItems[1]);

			KdPrint(("----------------------------------------------------------------------------------------------------\n"));

			for(ULONG Index=0;Index<SplayTreesGlobalTree.GetNumberOfItems();Index++)
			{
				PVOID											RawTreeItem=SplayTreesGlobalTree.GetItemByIndex(Index);
				...
				{
					KdPrint(("!!!!!!!!!! TREE [%p] ITEM RETRIEVED by INDEX FAILED. INDEX is OUT of BOUNDS.\n",&SplayTreesGlobalTree));
				}
			}
			
			KdPrint(("----------------------------------------------------------------------------------------------------\n"));

			BOOLEAN												IsEmptyTree=SplayTreesGlobalTree.IsEmpty();
			...
			while(true)
			{
				PVOID											RawTreeItem=SplayTreesGlobalTree.Enumerate(Restart);
		...
		while(true)
		{
			SMyTreeKey*											ItemToRemove=SplayTreesGlobalTree.GetItemByIndex(0);

			if (ItemToRemove!=nullptr)
			{
				SplayTreesGlobalDeleteItem(SplayTreesGlobalTree,*ItemToRemove);
			}
			else
			{
				break;
