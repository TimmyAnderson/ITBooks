...
//-------------------------------------------------------------------------------------------------------
		public bool ContainsSameLALRCore(CLALRCore LALRCore)
		{
			foreach(CLR0Item LALRCoreLR0Item in LALRCore.LR0Items)
			{
				...
				for(int Index=0;Index<MNFAStates.Count;Index++)
				{
					if (MNFAStates[Index].Item.LR0Item.Equals(LALRCoreLR0Item)==true)
					{
...
//-------------------------------------------------------------------------------------------------------
		public bool ContainsSameNFAStates(CNFAState[] NFAStates)
		{
			...
			for(int Index=0;Index<MNFAStates.Count;Index++)
			{
				if (MNFAStates[Index].Item.Equals(NFAStates[Index].Item)==false)
				{
					return(false);
				}
			}
...
//-------------------------------------------------------------------------------------------------------
		public bool ContainsSameNFAStates(CDFAState DFAState)
		{
			...
			for(int Index=0;Index<MNFAStates.Count;Index++)
			{
				if (MNFAStates[Index].Item.Equals(DFAState.NFAStates[Index].Item)==false)
				{
					return(false);
				}
			}
