...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
CPlaybackState::CPlaybackState(void)
	: MHead(), MLock(), MLookaside(), MCounter(), MStopEvent(), MThread(nullptr)
{
	KdPrint((DRIVER_PREFIX "CONSTRUCTOR [State::State()] CALLED.\n"));
...
CPlaybackState::~CPlaybackState(void)
{
	KdPrint((DRIVER_PREFIX "DESTRUCTOR [State::~State()] CALLED.\n"));
...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void CPlaybackState::PlayMelodyThread(PVOID Context)
{
	CPlaybackState*												Object=static_cast<CPlaybackState*>(Context);
...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void CPlaybackState::PlayMelody(void)
{
...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
NTSTATUS CPlaybackState::Start(PVOID IoObject)
{
...

	{
		...
		// !!!!! THREAD NEMOZE byt vytvoreny ako SYSTEM THREAD, pretoze by zvuk, ktory vydava BEEP DRIVER NEBOLO POCUT. Preto je nutne vytvorit THREAD v CURRENT CLIENT PROCESS, ktory poziadal o prehratie prveho SOUND.
		// !!!!! NEVYHODA tohto riesenia je, ze PRVY PROCESS, ktory poziadal o prehratie SOUND ostane v MEMORY, az kym nebude MELODY DRIVER UKONCENY, co rozhodne nie je dobre riesenie.
		NTSTATUS												Status=IoCreateSystemThread(IoObject,&MThread,THREAD_ALL_ACCESS,nullptr,NtCurrentProcess(),nullptr,PlayMelodyThread,this);

		return(Status);
	}
}
//----------------------------------------------------------------------------------------------------------------------
void CPlaybackState::Stop(void)
{
	// ????? NEDOCHADZA tu k RACE CONDITION s FUNCTION [Start()].
	if (MThread!=nullptr)
	{
...
//----------------------------------------------------------------------------------------------------------------------
NTSTATUS CPlaybackState::AddNotes(const SNote* Notes, ULONG Count)
{
