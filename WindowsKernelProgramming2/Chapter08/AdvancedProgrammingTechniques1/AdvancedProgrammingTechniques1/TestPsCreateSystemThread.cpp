...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
void ThreadPsCreateSystemThread(PVOID StartContext)
{
	...
	// !!!!! Ak je SYSTEM THREAD vytvoreny pomocou FUNCTION [PsCreateSystemThread()] musi VZDY na konci THREAD ROUTINE volat NON RETURNABLE FUNCTION [PsTerminateSystemThread()], aby mohol byt THREAD korektne uvolneny z MEMORY.
...
NTSTATUS TestPsCreateSystemThread(DEVICE_OBJECT* DeviceObject, IRP* Irp)
{
	...
	do
	{
		...
		// !!!!! NEVYHODOU pouzitia FUNCTION [PsCreateSystemThread()] je to ze NEGARANTUJE, ze WINDOWS neodstrani DRIVER z MEMORY skor ako SYSTEM THREAD skonci.
		Status=PsCreateSystemThread(&ThreadHandle,THREAD_ALL_ACCESS,&ThreadHandleObjectAttributes,nullptr,nullptr,ThreadPsCreateSystemThread,nullptr);
		...
		{
			// !!!!! Vracia STATUS CODE, ktory bol zaslany do FUNCTION [PsTerminateSystemThread()].
