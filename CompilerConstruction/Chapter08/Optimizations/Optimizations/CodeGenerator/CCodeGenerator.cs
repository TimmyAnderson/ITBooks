...
//-------------------------------------------------------------------------------------------------------
		private string ConvertStringLiteral(string StringLiteral)
		{
			...
			{
				char											Char=StringLiteral[Index];

				if (Char=='\\')
				{
					char										NextChar=StringLiteral[Index+1];
					...
					{
						throw(new CCompilerException("Invalid LITERAL !"));
					}
				}
				else
				{
...
//-------------------------------------------------------------------------------------------------------
		private CInstructionCollection RemoveUnnecessaryInstructions()
		{
			CInstruction[]										NonOptimizedInstructions=MInstructions.Instructions;
			List<CInstruction>									InstructionsAfterOptimalization1=new List<CInstruction>();
			
			// !!! Vyhodim dvojicu instrukcii PUSH EAX a POP EAX, ak idu IHNED ZA SEBOU.
			for (int Index=0;Index<NonOptimizedInstructions.Length;Index++)
			{
				bool											AddInstruction=true;
				CInstruction									CurrentInstruction=NonOptimizedInstructions[Index];

				if ((Index+1)<NonOptimizedInstructions.Length)
				{
					if ((CurrentInstruction is CInstruction_PUSH)==true)
					{
						CInstruction_PUSH						InstructionPUSH=(CInstruction_PUSH) CurrentInstruction;

						if ((InstructionPUSH.Operand is COperand_Register)==true)
						{
							COperand_Register					PushRegisterOperand=(COperand_Register) InstructionPUSH.Operand;

							if (PushRegisterOperand.Register==ERegister.E_R_EAX)
							{
								CInstruction					NextInstruction=NonOptimizedInstructions[Index+1];

								if ((NextInstruction is CInstruction_POP)==true)
								{
									CInstruction_POP			InstructionPOP=(CInstruction_POP) NextInstruction;

									if ((InstructionPOP.Register==ERegister.E_R_EAX))
									{
										AddInstruction=false;

										// Preskocim instrukciu POP.
										Index++;
									}
								}
							}
				...
				{
					InstructionsAfterOptimalization1.Add(CurrentInstruction);
				}
			}

			List<CInstruction>									InstructionsAfterOptimalization2=new List<CInstruction>();

			// !!! Nahradim instrukcie PUSH EAX a POP EBX za instrukciu MOV EBX,EAX.
			for (int Index=0;Index<InstructionsAfterOptimalization1.Count;Index++)
			{
				bool											AddInstruction=true;
				CInstruction									CurrentInstruction=InstructionsAfterOptimalization1[Index];

				if ((Index+1)<InstructionsAfterOptimalization1.Count)
				{
					if ((CurrentInstruction is CInstruction_PUSH)==true)
					{
						CInstruction_PUSH						InstructionPUSH=(CInstruction_PUSH) CurrentInstruction;
						...
						if ((InstructionPUSH.Operand is COperand_Register)==true)
						{
							...
							if (PushRegisterOperand.Register==ERegister.E_R_EAX)
							{
								CInstruction					NextInstruction=InstructionsAfterOptimalization1[Index+1];

								if ((NextInstruction is CInstruction_POP)==true)
								{
									CInstruction_POP			InstructionPOP=(CInstruction_POP) NextInstruction;

									if ((InstructionPOP.Register==ERegister.E_R_EBX))
									{
										CInstruction_MOV		InstructionMOV=new CInstruction_MOV(new COperand_Register(ERegister.E_R_EBX),new COperand_Register(ERegister.E_R_EAX),"!!! OPTIMIZATION - PUSH EAX and POP EBX were REPLACED with MOV EBX,EAX.");

										InstructionsAfterOptimalization2.Add(InstructionMOV);
				...
				if (AddInstruction==true)
				{
					InstructionsAfterOptimalization2.Add(CurrentInstruction);
				}
			}

			List<CInstruction>									InstructionsAfterOptimalization3=new List<CInstruction>();

			// !!! V postupnosti instrukcii JMP LABEL_1, LABEL_2 a LABEL_1 vyhodim instrukciu JMP LABEL_1.
			for (int Index=0;Index<InstructionsAfterOptimalization2.Count;Index++)
			{
				bool											AddInstruction=true;
				CInstruction									CurrentInstruction=InstructionsAfterOptimalization2[Index];

				if ((Index+2)<InstructionsAfterOptimalization2.Count)
				{
					if ((CurrentInstruction is CInstruction_JMP)==true)
					{
						CInstruction_JMP						InstructionJMP=(CInstruction_JMP) CurrentInstruction;
						CInstruction							NextInstruction=InstructionsAfterOptimalization2[Index+1];

						if ((NextInstruction is CInstruction_LABEL)==true)
						{
							CInstruction						NextNextInstruction=InstructionsAfterOptimalization2[Index+2];

							if ((NextNextInstruction is CInstruction_LABEL)==true)
							{
								CInstruction_LABEL				InstructionLABEL=(CInstruction_LABEL) NextNextInstruction;

								if (InstructionJMP.Label==InstructionLABEL.Label)
								{
									AddInstruction=false;
								}
							}
				...
				if (AddInstruction==true)
				{
					InstructionsAfterOptimalization3.Add(CurrentInstruction);
				}
			}

			List<CInstruction>									InstructionsAfterOptimalization4=new List<CInstruction>();

			// !!! V postupnosti instrukcii JMP LABEL a LABEL vyhodim instrukciu JMP LABEL.
			for (int Index=0;Index<InstructionsAfterOptimalization3.Count;Index++)
			{
				bool											AddInstruction=true;
				CInstruction									CurrentInstruction=InstructionsAfterOptimalization3[Index];

				if ((Index+1)<InstructionsAfterOptimalization3.Count)
				{
					if ((CurrentInstruction is CInstruction_JMP)==true)
					{
						CInstruction_JMP						InstructionJMP=(CInstruction_JMP) CurrentInstruction;
						CInstruction							NextInstruction=InstructionsAfterOptimalization3[Index+1];

						if ((NextInstruction is CInstruction_LABEL)==true)
						{
							CInstruction_LABEL					InstructionLABEL=(CInstruction_LABEL) NextInstruction;
				...
				if (AddInstruction==true)
				{
					InstructionsAfterOptimalization4.Add(CurrentInstruction);
				}
			}

			CInstruction[]										OptimizedInstruction=InstructionsAfterOptimalization4.ToArray();

			CInstructionCollection								OptimizedInstructionCollection=new CInstructionCollection(OptimizedInstruction);

			return(OptimizedInstructionCollection);
		}
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_VARIABLE_DECLARATION_Choice2(CParseTreeNode Node, CCodeGenerationTraverseItem TraverseItem)
		{
			...
			if (MCurrentTable.ContainsSymbolInCurrentOrParentScopes(SymbolName)==true)
			{
				...
				if (Variable.IsECXRegisterOccupied==true)
				{
					CInstruction_MOV							Instruction_MOV=new CInstruction_MOV(new COperand_Register(ERegister.E_R_ECX),new COperand_Register(ERegister.E_R_EAX),string.Format("[VARIABLE_DECLARATION -> TYPE_SPECIFIER id=CONST;] - Moves value of constant for variable [{0}] to ECX.",SymbolName));
			...
			{
				throw(new CCompilerException(string.Format("Can't find SYMBOL for NAME [{0}] !",SymbolName)));
			}
		}
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePreOrderAction_FUNCTION_DECLARATION(CParseTreeNode Node, CCodeGenerationTraverseItem TraverseItem)
		{
			...
			CInstruction_PUSH									Instruction_PUSH3=new CInstruction_PUSH(new COperand_Register(ERegister.E_R_ECX),string.Format("[VARIABLE -> [FUNCTION_DECLARATION -> TYPE_SPECIFIER id(PARAMS) COMPOUND_STATEMENT]] - Stores original value of ECX to STACK."));

			AddInstruction(Instruction_PUSH3);
			...
			{
				AddInstruction(new CInstruction_ASM_TEXT("MOV EAX,[EBP+16]",string.Format("Moves ADDRESS of INTEGER VALUE to EAX.")));
			...
			{
				AddInstruction(new CInstruction_ASM_TEXT("MOV EAX,[EBP+16]",string.Format("Moves ADDRESS of STRING VALUE to EAX.")));
...
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_COMPOUND_STATEMENT(CParseTreeNode Node, CCodeGenerationTraverseItem TraverseItem)
		{
			...
			if ((MCurrentTable is CSymbolTable_FunctionDeclaration)==true)
			{
				...
				CInstruction_POP								Instruction_POP1=new CInstruction_POP(ERegister.E_R_ECX,string.Format("[COMPOUND_STATEMENT -> {{LOCAL_DECLARATIONS STATEMENT_LIST}}] - Restores original value of ECX."));

				AddInstruction(Instruction_POP1);

				CInstruction_POP								Instruction_POP2=new CInstruction_POP(ERegister.E_R_EBX,string.Format("[COMPOUND_STATEMENT -> {{LOCAL_DECLARATIONS STATEMENT_LIST}}] - Restores original value of EBX."));

				AddInstruction(Instruction_POP2);

				CInstruction_POP								Instruction_POP3=new CInstruction_POP(ERegister.E_R_EBP,string.Format("[COMPOUND_STATEMENT -> {{LOCAL_DECLARATIONS STATEMENT_LIST}}] - Restores original value of EBP."));

				AddInstruction(Instruction_POP3);
...
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_EXPRESSION_STATEMENT_Choice1(CParseTreeNode Node, CCodeGenerationTraverseItem TraverseItem)
		{
			...
			CParseTreeNode										TreeNodeVariable=Node.GetChildTreeNodeBySymbolName(NAME_VARIABLE,0);
			CParseTreeNode										TreeNodeID=TreeNodeVariable.GetChildTreeNodeBySymbolName(NAME_ID,0);
			CToken_Identifier									TokenID=(CToken_Identifier) TreeNodeID.Token;
			string												SymbolName=TokenID.Lexema;
			...
			if (MCurrentTable.ContainsSymbolInCurrentOrParentScopes(SymbolName)==true)
			{
				...
				if (Variable.IsECXRegisterOccupied==true)
				{
					...
					CInstruction_MOV							Instruction_MOV=new CInstruction_MOV(new COperand_Register(ERegister.E_R_ECX),new COperand_Register(ERegister.E_R_EBX),string.Format("[EXPRESSION_STATEMENT -> VARIABLE=EXPRESSION;] - Moves value of EXPRESSION to VARIABLE."));
		...
		private void TraversePostOrderAction_VARIABLE_Choice1(CParseTreeNode Node, CCodeGenerationTraverseItem TraverseItem)
		{
			...
			if (MCurrentTable.ContainsSymbolInCurrentOrParentScopes(SymbolName)==true)
			{
				...
				if (SymbolType==ESymbolType.E_ST_INT || SymbolType==ESymbolType.E_ST_STRING)
				{
					if (Variable.IsECXRegisterOccupied==true)
					{
						// Ulozi hodnotu ID na STACK.
						CInstruction_PUSH						Instruction_PUSH=new CInstruction_PUSH(new COperand_Register(ERegister.E_R_ECX),string.Format("[VARIABLE -> id] - Pushs value of variable [{0}] to STACK.",SymbolName));

						AddInstruction(Instruction_PUSH);
					}
					// Lokalna premenna, alebo parameter.
					else if (Variable.IsGlobalVariable==false)
					{
						int										StackOffset=SymbolFromSymbolTable.StackOffset;
				...
				{
					throw(new CCompilerException(string.Format("Invalid TYPE for SYMBOL [{0}] !",SymbolName)));
				}
			}
			else
			{
...
//-------------------------------------------------------------------------------------------------------
		...
		private void TraversePostOrderAction_EXPRESSION_Choice1(CParseTreeNode Node, CCodeGenerationTraverseItem TraverseItem)
		{
			...
			if (COMPARE_OPERATION.ChildNodes.Length>0)
			{
				...
				{
					throw(new CCompilerException("BNF RULE [COMPARE_OPERATION -> <= | < | > | >= | == | !=] contains INVALID SYMBOL !"));
				}
			}
			else
			{
				throw(new CCompilerException("BNF RULE [COMPARE_OPERATION -> <= | < | > | >= | == | !=] has INVALID number of CHILD NODES !"));
			}
		...
		private void TraversePostOrderAction_FACTOR_Choice2(CParseTreeNode Node, CCodeGenerationTraverseItem TraverseItem)
		{
			...
			if (MCurrentTable.ContainsSymbolInCurrentOrParentScopes(SymbolName)==true)
			{
				...
				else
				{
					if (Variable.IsECXRegisterOccupied==true)
					{
						// !!! Nerobim NIC, pretoze EAX obsahuje uz HODNOTU a NIE ADRESU.
					}
					else
					{
			...
			if (ParamTypes!=null)
			{
				...
				if (MCurrentTable.ContainsSymbolInCurrentOrParentScopes(FunctionName)==true)
				{
					...
					if ((SymbolFromSymbolTable.SymbolTableData is CSymbolTableData_Function)==true)
					{
						...
						{
							throw(new CCompilerException(string.Format("Can't FIND FUNCTION TABLE for FUNCTION [{0}] !",FunctionName)));
						}
					}
					else
					{
						throw(new CCompilerException(string.Format("NODE doesn't contain VALID FUNCTION DECLARATION !",Node)));
					}
				}
				else
				{
					throw(new CCompilerException(string.Format("NODE doesn't contain FUNCTION DECLARATION !",Node)));
				}
			}
			else
			{
				throw(new CCompilerException(string.Format("NODE doesn't contain PARAM TYPES INFORMATION !",Node)));
			}
		}
//-------------------------------------------------------------------------------------------------------
		// Choice - [CONST -> num].
...
//-------------------------------------------------------------------------------------------------------
		public void GenerateCode(CNonterminalCollection NonterminalCollection, CParseTreeNode RootParseTreeNode, CInstructionCollection Instructions)
		{
			...
			foreach(CSymbolTableData_Variable Variable in MGlobalVariables)
			{
				...
				{
					throw(new CCompilerException("INVALID TYPE of GLOBAL VARIABLE !"));
				}
			}
			...
			// Vyradi nadbytocne instrukcie.
			CInstructionCollection								OptimizedInstructions=RemoveUnnecessaryInstructions();

			Instructions.FillInstructions(OptimizedInstructions);
		}
//-------------------------------------------------------------------------------------------------------
	}
//-------------------------------------------------------------------------------------------------------
