...
//------------------------------------------------------------------------------------------------------
		private void SimpleThreadThreadMethod()
		{
...
//------------------------------------------------------------------------------------------------------
		...
		private void CreateThread()
		{
			try
			{
				Console.WriteLine("THREAD in APP DOMAIN [{0} - {1}] will be EXECUTED !",AppDomain.CurrentDomain.Id,AppDomain.CurrentDomain.FriendlyName);

				Thread												NewThread=new Thread(SimpleThreadThreadMethod);
...
//-------------------------------------------------------------------------------------------------------
		public override void InitializeNewDomain(AppDomainSetup AppDomainInfo)
		{
			...
			// !!!!! Povolim OBSOLETE CAS SECURITY pre VSETKY APP DOMAIN, vratane DEFAULT APP DOMAIN.
			AppDomainInfo.SetCompatibilitySwitches(new string[]{"NetFx40_LegacySecurityPolicy"});
		}
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
		public void CreateNewDomainAndNewThread(string DomainName)
		{
			try
			{
				Console.WriteLine("!!! BEGIN CreateNewDomainAndNewThread() - APP DOMAIN [{0} - {1}] !",AppDomain.CurrentDomain.Id,AppDomain.CurrentDomain.FriendlyName);
				...
				Setup.AppDomainInitializerArguments=new string[]{DomainName.ToUpper()};
				Setup.DisallowCodeDownload=true;
				// !!!!! Kedze pouzivam OBSOLETE CAS, tak MUSIM povolit NetFx40_LegacySecurityPolicy FLAG.
				// !!!!! Tento je NUTNE POVOLIT aj na DEFAULT APP DOMAIN, v ktorom sa vytvaraju dalsie APP DOMAINs. Toto sa robi v METHOD InitializeNewDomain() VYSSIE v CODE.
				Setup.SetCompatibilitySwitches(new string[]{"NetFx40_LegacySecurityPolicy"});

				// !!!!! Kedze HOST PROTECTION sa vztahuje IBA na PARTIAL TRUSTED ASSEMBLIES, tak je treba zmenit APP DOMAIN, aby nemala FULL TRUST.
				...
				// !!!!! Kedze bol zaregistrovany CUSTOM APP DOMAIN MANAGER (CLASS CMyDomainAppManager), tak prave TENTO sa pouzije na vytvorenie APP DOMAIN.
				// !!!!! Kedze pre KAZDU APP DOMAIN sa vytvara aj instancia APP DOMAIN MANAGER, okrem APP DOMAIN sa vytvori aj NOVA INSTANCIA CLASS CMyDomainAppManager, ktory sluzi ako CUSTOM APP DOMAIN MANAGER.
				AppDomain										NewDomain=AppDomain.CreateDomain(DomainName,CustomEvidence,Setup);
				...
				// !!!!! Tym, ze volam APP DOMAIN cez APP DOMAIN MANAGER, tak kod METHOD PrintAppCurrentDomainName() sa vykona v NOVOVYTVORENEJ APP DOMAIN.
				// !!!!! Namiesto prechodu do NOVEJ APP DOMAIN VOLANIM PrintAppCurrentDomainName() by som mohol vytvorit NOVY THREAD a v NOM vykonat prechod do NOVEJ APP DOMAIN (volanim PrintAppCurrentDomainName()).
				DomainManagerForNewDomain.CreateThread();

				// !!! Po skonceni volania METHOD PrintAppCurrentDomainName() sa THREAD vrati do POVODNEJ APP DOMAIN v ktorej bezal PRED VOLANIM METHOD PrintAppCurrentDomainName().
				Console.WriteLine("!!! END CreateNewDomainAndNewThread() - APP DOMAIN [{0} - {1}] !",AppDomain.CurrentDomain.Id,AppDomain.CurrentDomain.FriendlyName);
			}
			catch(Exception E)
			{
				Console.WriteLine("APP DOMAIN [{0} - {1}] - EXCEPTION [{2}] !",AppDomain.CurrentDomain.Id,AppDomain.CurrentDomain.FriendlyName,E.Message);
