...
//-------------------------------------------------------------------------------------------------------
		public void AddTraverseItem(CParseTreeTraverseItem TraverseItem)
		{
			if ((TraverseItem.Symbol is CTerminal)==true)
			{
				...
				{
					throw(new CCompilerException(string.Format("TRAVERSE ITEM LIST already contains TERMINAL [{0}] !",TraverseItem.Symbol)));
				}
			}
			else
			{
				...
				if (ExistingTraverseItem!=null)
				{
					throw(new CCompilerException(string.Format("TRAVERSE ITEM LIST already contains NONTERMINAL CHOICE [{0}] !",TraverseItem.Choice)));
				}
			}
				...
				else
				{
					if (TraverseItem.Symbol.Equals(TreeNode.Symbol)==true)
					{
						if (TraverseItem.Choice.Symbols.Length==TreeNode.ChildNodes.Length)
						{
							...
							for(int Index=0;Index<TraverseItem.Choice.Symbols.Length;Index++)
							{
								if (TraverseItem.Choice.Symbols[Index].Equals(TreeNode.ChildNodes[Index].Symbol)==false)
								{
									Equal=false;
									break;
								}
