...
{
//----------------------------------------------------------------------------------------------------------------------
	public class APIControllerAttributeClientController : Controller
	{
//----------------------------------------------------------------------------------------------------------------------
		public async Task<ViewResult> TestPostValidName()
		{
			...
			HttpResponseMessage									Response=await Client.PostAsync("http://localhost:50000/api/APIControllerAttribute",RequestContent);
			...
			else
			{
				...
				if (Content.Length>0)
				{
					...
					// !!!!! V pripade, ze CONTROLLER zisti VALIDATION ERROR, je generovana HTTP RESPONSE MESSAGE s HTTP STATUS CODE 400, pricom CONTENT obsahuje instanciu CLASS [ValidationProblemDetails].
					ValidationProblemDetails 					ValidationDetails=JsonSerializer.Deserialize<ValidationProblemDetails>(Content,Options);
					List<CValidationError>						ValidationErrors=new List<CValidationError>();

					foreach(KeyValuePair<string,string[]> KeyValue in ValidationDetails.Errors)
					{
						...
						ValidationErrors.Add(new CValidationError(KeyValue.Key,Errors.ToArray()));
					}

					CValidationErrors							Model=new CValidationErrors(ValidationErrors.ToArray());
